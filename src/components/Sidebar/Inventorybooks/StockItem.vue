...
export default {
  ...
  methods: {
    ...
    routeFunction(id, l_name) {
      let routeName = "tally-style-stockitem-monthly-summary";

      if (routeName) {
        localStorage.setItem("GroupIndex", this.groupIndex);
        localStorage.setItem("screen", "Stockitem");
    ...
    //   "company_id":"6710b4f49b80a640eeb8d2c7", //required
    // "_userId": "66cc165a7488ec4e12a04f45",   //required
    // "searchTerm":"",
    // "requestFrom": "WEB"
      ...
      Api.getStockItemsListGo(data, headers)
        ...
        .catch((error) => {
          ...
          Common.getStatusForActionFailed(error.response.status, "ye wala");
        });
    },
  },
