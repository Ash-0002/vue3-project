<template>
  <div>
    <div class="modal-vue-tally">
      ...
      <div class="overlay2" @click="focusStockGroupSummary"></div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    closeModalCompany() {
      this.$emit("closeStockGroupSummary");
    },
    ...
    selectGroupRow(index) {
      if (this.groupIndex !== index) {
        ...
        this.routeFunction(
          ...
          this.displayedLedgerList[index].stockGroupName
        );
      }
    },

    routeFunction(guid, stockGroupName) {
      let routeName = "tally-style-stock-group-summary-id";

      if(!this.$props.redirect){
        this.$emit("closeStockGroupSummary", guid, stockGroupName);
        return;
      }

      if (routeName) {
        localStorage.setItem("GroupIndex", this.groupIndex);
        localStorage.setItem("screen", "StockGroupSummary");
        //console.log(guid, stockGroupName);
        this.$router.push({
          name: routeName,
          params: {
            id: guid,
    ...
    handleEnterKey(event) {
      ...
      this.routeFunction(
        ...
        this.displayedLedgerList[this.groupIndex].stockGroupName,
      );
    },
    ...
    getLedgers() {
      const data = {
        ...
        startLimit: 0,
        endLimit: 20, // Required
        searchTerm: "",
        orderBy: 1,
        sortBy: "stockGroupName",
        ...
        .then((response) => {
          ...
          if (responseData == "200") {
            ...
            const primaryGroupIndex = this.ledgersList.findIndex(
              (item) => item.stockGroupName === "#4; Primary"
            );

            if (primaryGroupIndex !== -1) {
              this.ledgersList[primaryGroupIndex].stockGroupName =
                this.ledgersList[primaryGroupIndex].stockGroupName.replace(
                  "#4; ",
                  ""
                );
