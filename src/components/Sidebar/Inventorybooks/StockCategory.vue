                ...
                <div class="row">
                  <div class="col-12">
                    <div class="company-popup-list-name-left">
                      <a href="javascript:void(0);" v-if="ledger.stockCategoryName === `#4; Primary`">
                        ♦️ {{ ledger.stockCategoryName }}
                      </a>
                      <a href="javascript:void(0);" v-else>
                        {{ ledger.stockCategoryName }}
                      </a>
                    </div>
                  </div>
                  <!-- <div class="col-6">
  ...
  methods: {
    ...
    selectGroupRow(index) {
      if (this.groupIndex !== index) {
        ...
        this.routeFunction(this.displayedLedgerList[index].guid ,this.displayedLedgerList[index].stockCategoryName);
      }
    },

    routeFunction(guid, stockCategoryName) {
      let routeName = "tally-style-stock-category-summary";

      if (routeName) {
        localStorage.setItem("GroupIndex", this.groupIndex);
        localStorage.setItem("screen", "StockCategories");
    ...
    handleEnterKey() {
      this.routeFunction(this.displayedLedgerList[this.groupIndex].guid ,this.displayedLedgerList[this.groupIndex].stockCategoryName);
    },
      ...
      Api.getItemsCategoriesByCompanyIdGo(data, headers)
        .then((response) => {
          ...
          if (responseData == "200") {
            ...
            this.ledgersList = this.ledgersList.filter((category) => category.stockCategoryName !== '#4; Not Applicable');
            // console.log(this.ledgersList, "filtered ledgers list");
            const primaryGroupIndex = this.ledgersList.findIndex(
              (item) => item.stockCategoryName === "#4; Primary"
            );

            if (primaryGroupIndex !== -1) {
              this.ledgersList[primaryGroupIndex].stockCategoryName = 
                this.ledgersList[primaryGroupIndex].stockCategoryName.replace(
                  "#4; ",
                  ""
                );
              const primaryGroup = this.ledgersList.splice(
                primaryGroupIndex,
                1
              )[0];
              this.ledgersList.unshift(primaryGroup);
            } else {
              const Receivables = {
                closingBalance: "",
                closingRate: "",                   
                ...
                hsnCode: "",                  
                parentGuid: "",
                stockCategoryName: "Primary",
              };
              this.ledgersList.unshift(Receivables);
            }
