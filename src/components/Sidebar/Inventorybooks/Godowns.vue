...
export default {
  ...
  methods: {
    ...
    routeFunction(id, groupName ) {
      let routeName = "tally-style-godown-summary";

      if (routeName) {
        localStorage.setItem("GroupIndex", this.groupIndex);
        localStorage.setItem("screen", "GodownSummary");
        localStorage.setItem("godownGuid", id);
      ...
      Api.getGodownsByCompanyIdGo(data, headers)
        .then((response) => {
          ...
          // if (responseData == "200") {
            ...
            this.displayedLedgerList.splice(0, 0, Receivables);
