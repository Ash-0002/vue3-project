  ...
  export default {
    ...
    methods: {
      ...
      handleEnterKey(event) {
        ...
        const item = this.displayedCostCentreList[this.groupIndex];

        this.$router.push({
          name: "tally-style-cost-centre-breakUp",
          query: {
            centerName: item.costCentreName
          }
        })
      },
      ...
      scrollToSelectedRow() {
        this.$nextTick(() => {
          ...
          const container = selectedRow.closest(".company-selection-popup-boxes overflow-ledger-indexing-height-set");
          if (!container) {
      ...
      searchedGroupName(name) {
        if (name.trim() == "") {
          ...
          if(!this.costCentreList){
            return;
          }
          const filteredGroupName = this.costCentreList.filter((ledger) => {
            return ledger.costCentreName.toLowerCase().includes(name.toLowerCase());
          });
      ...
      getCostCentres() {
        ...
        Api.getCostCentersGo(data, headers)
          .then((response) => {
