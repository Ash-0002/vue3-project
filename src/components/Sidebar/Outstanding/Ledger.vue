...
export default {
  ...
  methods: {
    ...
    selectGroupRow(index) {
      if (this.groupIndex !== index) {
        ...
        const selectedLedger = this.displayedLedgerList[index];
        this.$store.commit("tallyStore/setCurrentLedgerName", selectedLedger.ledgerName);
        this.searchGroup = selectedLedger.ledgerName;
        this.routeFunction(selectedLedger.guid, selectedLedger.ledgerName);
      }
    },

    routeFunction(id, l_name) {
      const routeName = "tally-style-ledger-outstanding";
      ...
      if (routeName) {
        ...
        if (
          ...
          this.$route.params.guid !== targetRoute.params.guid ||
          this.$route.params.ledgerName !== targetRoute.params.ledgerName
        ) {
    ...
    handleEnterKey() {
      const selectedLedger = this.displayedLedgerList[this.groupIndex];
      this.$store.commit("tallyStore/setCurrentLedgerName", selectedLedger.ledgerName);
      this.searchGroup = selectedLedger.ledgerName;
      this.routeFunction(
        selectedLedger.guid, selectedLedger.ledgerName
      );
    },
    ...
    searchedGroupName(name) {
      if (name.trim() == "") {
        ...
        if (filteredGroupName.length > 0) {
          ...
          const selectedLedger = filteredGroupName[0]; // Assuming the first result is the default selection
          this.$store.commit("tallyStore/setCurrentLedgerName", selectedLedger.ledgerName);
        } else {
          // Clear the Vuex store if no matches
          this.$store.commit("tallyStore/setCurrentLedgerName", "");
        }
      }
    },
  ...
  mounted() {
    ...
    this.searchGroup = this.$store.state.tallyStore.currentLedgerName || "";
