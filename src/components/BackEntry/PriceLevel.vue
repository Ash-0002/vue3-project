<template>
    <div class="voucher_ledger_popup PriceLevelPopupSet">
        <h5>List of {{ listTitle }}</h5>
        <ul id="itemList" class="">
            <li v-for="(list, index) in priceLevelList" :key="index" :class="{ selected: selectedIndex === index }"
                @click="handleItemClick(index)" @dblclick="handleEnterKey">
                {{ list.priceLevelName || list.name }}
            </li>
        </ul>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        handleEnterKey() {
            if (this.selectedIndex >= 0 && this.selectedIndex < this.priceLevelList.length) {
                this.$emit('priceLevel-selected', this.priceLevelList[this.selectedIndex].priceLevelName || this.priceLevelList[this.selectedIndex].name);
            }
        },

        changeGroupIndex(direction) {
            const newIndex = this.selectedIndex + direction;
            if (newIndex >= 0 && newIndex < this.priceLevelList.length) {
                this.selectedIndex = newIndex;
            }
            // this.scrollToSelectedRow();
        },
