        ...
        <div class="col-lg-6 col-md-6 col-6">
          <div class="title_text" v-if="this.$route.params.voucher_id">
            <div v-if="isTransfer != '0'">
              <h4 class="page-title mb-0">Purchase Order Voucher</h4>
            </div>
            <div v-else>
              <h4 class="page-title mb-0">Update Purchase Order Voucher</h4>
            </div>
          </div>
          <div class="title_text" v-else>
            <h4 class="page-title mb-0">Create Purchase Order Voucher</h4>
          </div>
        </div>
          ...
          <div class="row">
            ...
            <div class="col-lg-6 col-12">
              ...
              <div class="form-group input_new_textbox mb-4">
                <input @input="html_tag_notaccept('voucherNumber', $event)"
                  type="text"
                      ...
                      <tr v-for="(item, index) in tableRows" :key="item.id">
                        ...
                        <td width="18%">
                          <select
                            ...
                            @change="amount_calculation(index)"
                          >
                        ...
                        <td>
                          <input @input="html_tag_notaccept('item_description', $event)"
                            type="text"
                            :disabled="disabled === '1' || isTransfer == '1'"
                  ...
                  <div class="editing_table_for_voucher" id="tax_div_move">
                    <table class="new_voucher_table">
                      <tr>
                        ...
                        <th
                          style="text-align: right"
                          @click.stop="tax_insert_Row" v-if="isTransfer == '0' || isTransfer == '' || isTransfer == null || isTransfer == '-1'"
                        >
                  ...
                  <div class="showledgers_table sales_cstm_dropdown" v-if="showledgers">
                    <div class="editing_table_for_voucher">
                      <table class="new_voucher_table">
                        ...
                        <tr
                          v-for="(ledger, index) in ledger_array"
                          :key="ledger.id"
                        >
                          <td>
                          ...
                          <td>
                            <select
                              ...
                              id=""
                              :disabled="isTransfer == '1'"
                              class="form-select item_select"
                              v-model="ledger.type"
                              @change="tax_calculation()"
                            >
                    ...
                    <div>
                      ...
                      <div class="sales-voucher-textarea">
                        <textarea @input="html_tag_notaccept('narration', $event)"
                          ...
                          cols="50" class="textarea-disable-not-allowed"
                          v-model="narration"
                        ></textarea>
                      </div>
                    </div>
              ...
              <div class="col-lg-6">
                <table width="100%" class="bottom_total_sales">
                  ...
                  <tr>
                    ...
                    <th style="text-align: right">
                      <span class="font_sans">{{ this.symbol }}</span>
                      {{ isNaN(parseFloat(this.gross_amount.toFixed(2))) ? 0 : parseFloat(this.gross_amount.toFixed(2)) }}
                    </th>
                  </tr>
                </table>
              </div>
...
export default {
   ...
   if(this.isValidGSTIN == false || this.isValidConsigneeGSTIN == false ){
    ...
    partySelected(selection, type="") {
      ...
      this.buyer_place_of_supply = selection.stateName;
        ...
        this.order_details_address = selection.address;
    ...
    tax_value_select(event, taxindex = 0, ledgerarray={}) {
      ...
      if (Object.keys(ledgerarray).length !== 0) {
        ...
        this.final_ledgerEntries.push(this.final_part_ledgerEntries["0"]);
    ...
    add_sales() {
      ...
      this.tableRows.forEach((value, key) => {
        ...
        if (value.rate == "" || value.rate == "0" || value.rate == "0.00") {
        ...
        .then((response) => {
          ...
          setTimeout(() => {
            ...
            for (let [key1, value1] of Object.entries(response.data.data.ledgerEntries)) {
                this.final_part_ledgerEntries = [];
                response.data.data.ledgerEntries[key1].amount = Math.abs(response.data.data.ledgerEntries[key1].amount)
    ...
    InvoiceConfigurations(){
      ...
      Api.getInvoiceConfigurationsGo(data, headers)
        .then((response) => {
          if (response.data.status != "0") {
            this.invoice_type = response.data.data.vouchers.voucherNumberConfiguration.purchaseorder.isSetDefaultNumber;
          }
        })
        .catch((err) => {
          Common.getStatusForActionFailed(err.response.status)
  ...
  mounted() {
    GoogleAnalytics.create_purchase_order_voucher_listing();
