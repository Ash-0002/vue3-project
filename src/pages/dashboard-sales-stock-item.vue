              ...
              <v-app id="inspire">
                ...
                >
                  <v-data-table
                    ...
                    :items="salecustomer"
                    :options.sync="options"
                      :items-per-page="this.$store.state.Web1_Store.Sales_StockItem_page"
                    :server-items-length="totalsalecustomer"
                    :loading="loading"
                    class="elevation-1"
                    must-sort
...
export default {
  ...
  data() {
    ...
    return {
      ...
      page: this.$store.state.Web1_Store.Sales_StockItem_page,
      totalsalecustomer: 0,
      numberOfPages: 0,
      salecustomer: [],
      value: [],
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.sales_stock_item_screen_pagination(this.options.page);
    },

    "options.itemsPerPage"() {
     
      const StartIndex = (this.$store.state.Web1_Store.Sales_StockItem_page - 1) * this.$store.state.Web1_Store.Sales_StockItem_itemsPerPage
      const endIndex = StartIndex +  this.$store.state.Web1_Store.Sales_StockItem_itemsPerPage 
      const Difference = endIndex - this.totalsalecustomer
console.log(Difference, "hi difference this side")
      setTimeout(() => {
     ...
     this.page= this.$store.state.Web1_Store.Sales_StockItem_page
     this.options.page = this.page; 
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.sales_stock_item_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.sales_stock_item_screen_listing();
      },
    },
    deep: true,
  },
  ...
  computed:{
    currentPage() {
      return this.$store.state.Web1_Store.Sales_StockItem_page;
    },

  },
  methods: {
    freeusergeneratePdf() {
      GoogleAnalytics.sales_stock_item_screen_viewpdf("viewpdf");
    },
    StoredPage(newValue){
      this.$store.dispatch('Web1_Store/setSalesStockItemPage', newValue.page)
      

    },
    updateItemsPerPage(newValue){
      this.$store.dispatch('Web1_Store/setSalesStockItemItemsPerPage', newValue)
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.sales_stock_item_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.sales_stock_item_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.sales_stock_item_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.sales_stock_item_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.sales_stock_item_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.sales_stock_item_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.sales_stock_item_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    left_arrow() {
      ...
      GoogleAnalytics.sales_screen_datepicker(
          ...
          .then((response) => {
            ...
            if (responseStatus == "200") {
                for (let [key, value] of Object.entries(
                  ...
                  response.data.data.list[key].totalQuantity = parseInt(response.data.data.list[key].totalQuantity)
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.sales_stock_item_screen_search(this.search);
    ...
    updateSort(byDesc, event) {
      ...
      this.sale_data("", this.groupSelectValue, this.switch_type);
      GoogleAnalytics.sales_stock_item_screen_sorting(
        this.sortBy,
        this.orderBy
      );
    ...
    update() {
      ...
      GoogleAnalytics.sales_stock_item_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
