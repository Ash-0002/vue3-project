              ...
              <v-app id="inspire">
                ...
                >
                  <v-data-table
                    ...
                    :options.sync="options"
                    :items-per-page="
                      this.$store.state.Web1_Store.Payment_Bill_itemsPerPage
                    "
                    :server-items-length="totalsalecustomer"
                    :loading="loading"
                    class="elevation-1"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      page: this.$store.state.Web1_Store.Payment_Bill_page,
      totalsalecustomer: 0,
      numberOfPages: 0,
      salecustomer: [],
      value: [],
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.payment_bill_screen_pagination(this.options.page);
    },
    ...
    "options.itemsPerPage"() {
      ...
      setTimeout(() => {
        const StartIndex =
          (this.$store.state.Web1_Store.Payment_Bill_page - 1) *
          this.$store.state.Web1_Store.Payment_Bill_itemsPerPage;
        const endIndex =
          StartIndex + this.$store.state.Web1_Store.Payment_Bill_itemsPerPage;
        ...
        if (Difference > this.totalsalecustomer) {
          ...
          this.page = this.$store.state.Web1_Store.Payment_Bill_page;
          this.options.page = this.page;
        }
      }, 1000);
      this.sale_data();
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.payment_bill_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.payment_bill_listing();
      },
    },
    deep: true,
  },
  ...
  computed: {
    currentPage() {
      return this.$store.state.Web1_Store.Payment_Bill_page;
    },
  },
  methods: {
    freeusergeneratePdf() {
      GoogleAnalytics.payment_bill_screen_viewpdf("viewpdf");
    },
    StoredPage(newValue) {
      this.$store.dispatch("Web1_Store/setPaymentBillPage", newValue.page);
    },
    updateItemsPerPage(newValue) {
      this.$store.dispatch("Web1_Store/setPaymentBillItemsPerPage", newValue);
    },
    VoucherPdf(v_id){
      const data = {
        requestFrom: "WEB",
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.payment_bill_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.payment_bill_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.payment_bill_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.payment_bill_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.payment_bill_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.payment_bill_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.payment_bill_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    updateSort(byDesc, event) {
      ...
      GoogleAnalytics.payment_bill_screen_sorting(this.sortBy, this.orderBy);
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.payment_bill_screen_search(this.search);
    ...
    update() {
      ...
      GoogleAnalytics.payment_bill_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
