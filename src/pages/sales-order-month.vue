              ...
              <v-app id="inspire">
                ...
                >
                  <v-data-table
                    ...
                    :items="salecustomer"
                    :options.sync="options"
                    :server-items-length="totalsalecustomer1"
                    :loading="loading"
                    class="elevation-1"
                    must-sort
                    :sort-by="null"
...
export default {
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.salesorder_month_screen_pagination(this.options.page);
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.salesorder_month_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.salesorder_month_listing();
      },
    },
    deep: true,
  },
  ...
  methods: {
    ...
    freeusergeneratePdf() {
      GoogleAnalytics.salesorder_month_screen_viewpdf("viewpdf");
    },
    VoucherPdf(v_id){
      const data = {
        requestFrom: "WEB",
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.salesorder_month_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.salesorder_month_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.salesorder_month_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.salesorder_month_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.salesorder_month_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.salesorder_month_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.salesorder_month_screen_date_filter("Max");
        var new_date_start_date = moment(
            ...
            if (response.data.message == "AUTHENTICATION_FAILED") {
              ...
              if (response.data.status != "0") {
                for (let [key, value] of Object.entries(
                  ...
                  response.data.data.list[key].url = "/add-parties";
                  response.data.data.list[key].amount = Math.abs(
                    response.data.data.list[key].amount
                  );
             
                  response.data.data.list[key].monthYear = this.formatDate(response.data.data.list[key].monthYear);
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.salesorder_month_screen_search(this.search);
    ...
    updateSort(byDesc, event) {
      ...
      GoogleAnalytics.salesorder_month_screen_sorting(
        this.sortBy,
        this.orderBy
      );
    ...
    update() {
      ...
      GoogleAnalytics.salesorder_month_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
