              ...
              <v-app id="inspire">
                ...
                >
                  <v-data-table
                    ...
                    :items="salecustomer"
                    :options.sync="options"
                    :items-per-page="this.$store.state.Web1_Store.Sales_SalesOrder_StockItem_itemsPerPage"
                    :server-items-length="totalsalecustomer"
                    :loading="loading"
                    class="elevation-1"
                    must-sort
...
export default {
  ...
  data() {
    ...
    return {
      ...
      page: this.$store.state.Web1_Store.Sales_SalesOrder_StockItem_page,
      totalsalecustomer: 0,
      numberOfPages: 0,
      salecustomer: [],
      value: [],
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.salesorder_stock_item_screen_pagination(
        this.options.page
      );
    },
    ...
    "options.itemsPerPage"() {
      ...
      setTimeout(() => {
      const StartIndex = (this.$store.state.Web1_Store.Sales_SalesOrder_StockItem_page - 1) * this.$store.state.Web1_Store.Sales_SalesOrder_StockItem_itemsPerPage
      const endIndex = StartIndex +  this.$store.state.Web1_Store.Sales_SalesOrder_StockItem_itemsPerPage 
     ...
     this.page= this.$store.state.Web1_Store.Sales_SalesOrder_StockItem_page
     this.options.page = this.page; 
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.salesorder_stock_item_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.salesorder_stock_item_listing();
      },
    },
    deep: true,
  },
  ...
  computed:{
    currentPage() {
      return this.$store.state.Web1_Store.Sales_SalesOrder_StockItem_page;
    },

  },
  methods: {
    freeusergeneratePdf() {
      GoogleAnalytics.salesorder_stock_item_screen_viewpdf("viewpdf");
    },
    StoredPage(newValue){
      this.$store.dispatch('Web1_Store/setSalesSalesOrderStockItemPage', newValue.page)
      

    },
    updateItemsPerPage(newValue){
      this.$store.dispatch('Web1_Store/setSalesSalesOrderStockItemItemsPerPage', newValue)
    ...
    activate: function (el) {
      ...
      if (el === "Ledger") {
        ...
        this.$router.push({ name: "sales-order-stock-item" });
      } else if (el === "Month") {
        this.$router.push({ name: "sales-order-month" });
      } else if (el === "Bill") {
        this.$router.push({ name: "sales-order-bill" });
      } else if (el === "Ledger Group") {
        this.$router.push({ name: "sales-order-ledger-group" });
      } else if (el === "Voucher Type") {
        this.$router.push({ name: "sales-order-voucher-type-details" });
      } else if (el === "Stock Category") {
        this.$router.push({ name: "sales-order-stock-category" });
      } else if (el === "Stock Group") {
        this.$router.push({ name: "sales-order-stock-group" });
      }
      this.active_el = el;
      this.groupSelectValue = el;
      this.sale_data("", el, this.switch_type);
      GoogleAnalytics.salesorder_screen_filter(el, this.switch_type);
    },
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.salesorder_stock_item_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.salesorder_stock_item_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.salesorder_stock_item_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.salesorder_stock_item_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.salesorder_stock_item_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.salesorder_stock_item_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.salesorder_stock_item_screen_date_filter("Max");
        var new_date_start_date = moment(
          ...
          .then((response) => {
            ...
            if (responseStatus == "200") {
                for (let [key, value] of Object.entries(
                  ...
                  response.data.data.list[key].grandTotalAmount = Math.abs(
                    response.data.data.list[key].grandTotalAmount
                  );
                }
                ...
                this.totalAmount = Math.abs(
                  response.data.data.grandTotalAmount
                );
    ...
    groupSelect(event) {
      ...
      GoogleAnalytics.salesorder_screen_filter(
        event.target.value,
        this.switch_type
      );
    },
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.salesorder_stock_item_screen_search(this.search);
    ...
    updateSort(byDesc, event) {
      ...
      GoogleAnalytics.salesorder_stock_item_screen_sorting(
        this.sortBy,
        this.orderBy
      );
    ...
    update() {
      ...
      GoogleAnalytics.salesorder_stock_item_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
