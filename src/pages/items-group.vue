                        ...
                        <tr>
                          <td class="custom_table_a">
                            <router-link
                              ...
                              :to="{
                                ...
                                params: {
                                  guid: item.guid,
                                  group_name: item.stockGroupName,
                                },
                              }"
                            >
                              {{
                                handleSymbol(item.stockGroupName)
                              }}</router-link
                            >
                          </td>
                          <td class="">
                          ...
                          <td class="">
                            <p>
                              ...
                              {{ Math.abs(item.closingValue) | formatNumberParties }}
                              <span v-if="item.closingValue < 0" >Dr</span>
                            <span v-else-if="item.closingValue > 0" >Cr</span>
                            <span v-else ></span>
...
export default {
  ...
  watch: {
    page: function () {
      ...
      this.groupAPI();
      GoogleAnalytics.items_group_screen_pagination(this.options.page);
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.items_group_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        GoogleAnalytics.items_group_listing();
      },
    },
    deep: true,
  },
  ...
  methods: {
    ...
    freeuseraddnew() {
      GoogleAnalytics.items_group_screen_addnew("addnew");
    },
    paiduseraddnew() {
      GoogleAnalytics.items_group_screen_addnew("addnew");
    },
    freeusergeneratePdf() {
      GoogleAnalytics.items_group_screen_viewpdf("viewpdf");
    },
    generatePdf() {
      GoogleAnalytics.items_group_screen_viewpdf("viewpdf");
      const data = {
        ...
        orderBy: parseInt(this.orderByGroup),
        requestFrom: "WEB",
      };
      const headers = {
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.items_group_screen_search(this.search);
      this.groupAPI(type);
    },
    updateSortGroup(byDesc, event) {
      ...
      this.groupAPI();
      GoogleAnalytics.items_group_screen_sorting(
        this.sortByGroup,
        this.orderByGroup
      );
    },
