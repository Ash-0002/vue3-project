                ...
                <div class="col-xl-3 col-lg-12 col-12">
                    <div class="company_name sales_c_name mobile_padding_left_0">
                                <div class="company_balance sales_cb new_title_sales_cb">
                                    <p>Total Purchase Order: <br><strong><span class="font_sans">{{symbol}}</span> {{totalAmount | formatNumberSales}}</strong></p>
                                </div>
                            </div>
                </div>
                    ...
                    <ul class="new_set_dropdown_with_gross">
                        <li class="marginright25">
                             ...
                             <div class="grossnet_section">
                                        <div class="switches-container new_design_grossnet">
                                            ...
                                            <input type="radio" :disabled="inputDisabled" id="switchYearly"
                                                @change="switch_change($event)" v-model="switch_type" name="switchPlan"
                                                value="Net" />
                                            <label for="switchMonthly" v-b-tooltip.hover.top="'Gross'">GR</label>
                                            <label for="switchYearly" v-b-tooltip.hover.top="'Net'">NT</label>
                            ...
                            <li class="marginright25">
                                ...
                                <li @click="activate('Ledger')" :class="{ active : active_el == 'Ledger' }" v-b-tooltip.hover.top="'Ledger'">LED</li>
                                <li @click="activate('Stock Item')" :class="{ active : active_el == 'Stock Item' }" v-b-tooltip.hover.top="'Stock Item'">STIM</li>
                                <li @click="activate('Month')" :class="{ active : active_el == 'Month' }" v-b-tooltip.hover.top="'Month'">MON</li>
                                <li @click="activate('Bill')" :class="{ active : active_el == 'Bill' }" v-b-tooltip.hover.top="'Bill'">BILL</li>
                                ...
                                <div v-if="showInside" v-click-outside="closeEvent" class="without_inline_filter">
                                            <ul class="dropdown_li_active">
                                                <li @click="activate('Ledger Group')" v-click-outside="closeEvent" :class="{ active : active_el == 'Ledger Group' }">Ledger Group</li>
                                                <li @click="activate('Voucher Type')" v-click-outside="closeEvent" :class="{ active : active_el == 'Voucher Type' }">Voucher Type</li>
                                                <li @click="activate('Stock Group')" v-click-outside="closeEvent" :class="{ active : active_el == 'Stock Group' }">Stock Group</li>
                                                <li @click="activate('Stock Category')" v-click-outside="closeEvent" :class="{ active : active_el == 'Stock Category' }">Stock Category</li>
                                            </ul>              
                                      </div>
                                </ul>
                            </li>
                        ...
                        <img src="/assets/images/1.0/png/white-add.png" alt="img" class="table-icon-add" /> Add New
                                ...
                                <div v-if="all_view" class="show_entries_for_table new_show_entries_for_table">
                                    <v-data-table :page="page" :pageCount="numberOfPages" :headers="selectedHeaders"
                                        ...
                                        ref="table" disable-pagination hide-default-footer= true :footer-props="{'items-per-page-options':[10,25,50,100],'items-per-page-text':'products per page'}">
                                        ...
                                        <template v-slot:item.name="{ item }">
                                            <td><router-link :to="`${item.url}`" class="onclick_details">{{ item.name }}</router-link></td>
                                        </template>
                                ...
                                <div v-if="bill_view" class="show_entries_for_table new_show_entries_for_table">
                                    <v-data-table :page="page" :pageCount="numberOfPages" :headers="selected_bill_headers"
                                        :items="salecustomer" :options.sync="options"
                                        :server-items-length="totalsalecustomer" :loading="loading" class="elevation-1"
                                        must-sort @update:sort-by="updateSort('by', $event)"
                                        @update:sort-desc="updateSort('desc', $event)" item-key="table_header_index"
                                        ...
                                        <template v-slot:item.date="{ item }">
                                            <td><router-link :to="`${item.url}`" class="onclick_details">{{ item.date }}</router-link></td>
                                        </template>
                                        ...
                                        <template v-slot:item.voucherNumber="{ item }">
                                            <td>
                                                <p><router-link :to="`${item.url}`" class="onclick_details">{{item.voucherNumber}}</router-link>
                                                </p>
                                            </td>
                                        </template>
                                        <template v-slot:item.partyLedgerName="{ item }">
                                            <td>
                                                <p>{{ item.partyLedgerName}}
                                                </p>
                                            </td>
                                        </template>
                                        <template v-slot:item.totalAmount="{ item }">
                                            <td align="right">
                                                <p>{{ item.totalAmount | formatNumberSales}}
                                                </p>
                                            </td>
                                        </template>
                                    </v-data-table>
                                ...
                                <div v-if="month_view" class="show_entries_for_table new_show_entries_for_table">
                                    <v-data-table :page="page" :pageCount="numberOfPages" :headers="selectedHeaders"
                                        ...
                                        <template v-slot:item.name="{ item }">
                                            <td><button class="button_onclick" @click="monthClick(item.startDate,item.endDate)" >{{ item.name }}</button></td>
                                        </template>
                                        <template v-slot:item.totalAmount="{ item }">
                                            <td align="right">
                                                <p><span class="font_sans">{{symbol}}</span> {{ item.totalAmount | formatNumberSales }}
                                ...
                                <div v-if="voucher_type_view" class="show_entries_for_table new_show_entries_for_table">
                                    <v-data-table :page="page" :pageCount="numberOfPages" :headers="selectedHeaders"
                                        :items="salecustomer" :options.sync="options" 
                                        :server-items-length="totalsalecustomer" :loading="loading" class="elevation-1"
                                        must-sort @update:sort-by="updateSort('by', $event)"
                                        ...
                                        <template v-slot:item.name="{ item }">
                                            ...
                                            <td><button @click="vouchertypeClick(item.name)" class="button_onclick" >{{ item.name }}</button></td>
                                        </template>
                                        <template v-slot:item.totalAmount="{ item }">
                                            <td align="right">
                                                <p class="amount_closing"><span class="font_sans">{{symbol}}</span> {{ item.totalAmount | formatNumberSales }}
                                                </p>
                                            </td>
                                        </template>
                                        <!-- <template v-slot:item.totalQty="{ item }">
...
export default {
    ...
    watch: {
        ...
        options: {
            handler() {
                ...
                GoogleAnalytics.purchases_order_listing();
            },
        },
        deep: true,
    },
    ...
    methods: {
        activate:function(el){
            if(el === 'Ledger Group' || el === 'Voucher Type' || el === 'Stock Group' || el === 'Stock Category'){
                this.dotshow = 1;
            }else{
                this.dotshow = 0;
            }
    ...
    show(){
        ...
        left_arrow() {
            ...
            localStorage.setItem("date_range_start_date", new_date_start_date);
            localStorage.setItem("date_range_end_date", new_date_end_date);
            GoogleAnalytics.purchaseorder_screen_datepicker(moment(String(new_date_start_date)).format('yyyy-MM-DD'),moment(String(new_date_end_date)).format('yyyy-MM-DD'));
            this.sale_data("",this.groupSelectValue,this.switch_type);
        ...
        sale_data(type,group_name="Ledger",switch_type ="Gross",v_type="") {
                        ...
                        if(response.data.status != "0"){
                            for (let [key, value] of Object.entries(response.data.data.list)) {
                                if(response.data.data.list[key].stockGroupChild == false){
                                    response.data.data.list[key].url = '/stock-group-item/'+response.data.data.list[key].stockGroupGuid + '/'+btoa(response.data.data.list[key].name)+'/Purchase Order';
                                }else{
                                    response.data.data.list[key].url = '/stock-group/'+response.data.data.list[key].stockGroupGuid + '/'+btoa(response.data.data.list[key].name)+'/Purchase Order';
                                }
            ...
            if(this.groupSelectValue === "Ledger"){
                ...
                Api.getVouchersSummaryByLedgerList(data, headers)
                .then((response) => {
                    if (response.data.message == "AUTHENTICATION_FAILED") {
                        localStorage.clear();
                        window.location.href = "/";
                        ...
                        if(response.data.status != "0"){
                            for (let [key, value] of Object.entries(response.data.data.list)) {
                                response.data.data.list[key].url = '/parties' + '/' + response.data.data.list[key].ledgerGuId + '/Purchase Order';
                                response.data.data.list[key].totalAmount = Math.abs(response.data.data.list[key].totalAmount);
                            }
                            let totalPages = Math.ceil(
                                response.data.data.totalCount / itemsPerPage
                        ...
                        if(response.data.status != "0"){
                            for (let [key, value] of Object.entries(response.data.data.list)) {
                                if(response.data.data.list[key].ledgerGroupChild == false){
                                    response.data.data.list[key].url = '/ledger-group-ledger/'+response.data.data.list[key].ledgerGroupGuid + '/'+btoa(response.data.data.list[key].name)+'/Purchase Order';
                                }else{
                                    response.data.data.list[key].url = '/ledger-group/'+response.data.data.list[key].ledgerGroupGuid + '/'+btoa(response.data.data.list[key].name)+'/Purchase Order';
                                }
                                response.data.data.list[key].totalAmount = Math.abs(response.data.data.list[key].totalAmount);
                            }
                            let totalPages = Math.ceil(
            ...
            if(this.groupSelectValue === "Voucher Type"){
                ...
                Api.getVouchersSummaryByVoucherTypeList(data, headers)
                .then((response) => {
                    if (response.data.message == "AUTHENTICATION_FAILED") {
                        localStorage.clear();
                        window.location.href = "/";
            ...
            if(this.groupSelectValue === "Stock Category"){
                ...
                .then((response) => {
                    if (response.data.message == "AUTHENTICATION_FAILED") {
                        ...
                        if(response.data.status != "0"){
                            for (let [key, value] of Object.entries(response.data.data.list)) {
                                if(response.data.data.list[key].stockCategoryChild == false){
                                    response.data.data.list[key].url = '/stock-category-item/'+response.data.data.list[key].stockCategoryGuid + '/'+btoa(response.data.data.list[key].name)+'/Purchase Order';
                                }else{
                                    response.data.data.list[key].url = '/stock-category/'+response.data.data.list[key].stockCategoryGuid + '/'+btoa(response.data.data.list[key].name)+'/Purchase Order';
                                }
            ...
            if(this.groupSelectValue === "Stock Item"){
                ...
                .then((response) => {
                    if (response.data.message == "AUTHENTICATION_FAILED") {
                        ...
                        if(response.data.status != "0"){
                            for (let [key, value] of Object.entries(response.data.data.list)) {
                                response.data.data.list[key].url = '/items/' + response.data.data.list[key].stockItemGuid;
                                response.data.data.list[key].totalAmount = Math.abs(response.data.data.list[key].totalAmount);
                            }
                            let totalPages = Math.ceil(
                                response.data.data.totalCount / itemsPerPage
            ...
            if(this.groupSelectValue === "Bill"){
                ...
                .then((response) => {
                    if (response.data.message == "AUTHENTICATION_FAILED") {
                        ...
                        if(response.data.status != "0"){
                            for (let [key, value] of Object.entries(response.data.data.list)) {
                                response.data.data.list[key].url = '/parties/details/'+ response.data.data.list[key]._id + '/'+response.data.data.list[key].parentType;
                                response.data.data.list[key].totalAmount = Math.abs(response.data.data.list[key].totalAmount);
        ...
        groupSelect(event){
            ...
            GoogleAnalytics.purchaseorder_screen_filter(event.target.value,this.switch_type);
        },
        switch_change(event){
            this.sale_data("",this.groupSelectValue,event.target.value);
            GoogleAnalytics.purchaseorder_screen_filter(this.groupSelectValue,event.target.value);
        },
        updateSort(byDesc, event) {        
            ...
            GoogleAnalytics.purchaseorder_screen_sorting(this.sortBy,this.orderBy);
        ...
        handleBlur() {
            const type = "search";
            GoogleAnalytics.purchaseorder_screen_search(this.search);
        ...
        update() {
            ...
            GoogleAnalytics.purchaseorder_screen_datepicker(moment(String(this.$refs.picker.start)).format('yyyy-MM-DD'),moment(String(this.$refs.picker.end)).format('yyyy-MM-DD'));
            this.sale_data("",this.groupSelectValue,this.switch_type);
            this.date_setting();
        },
        date_setting(){
