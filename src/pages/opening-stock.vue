                    ...
                    <div class="row align-items-center">
                        <div class="col-lg-6 col-12">
                            <div class="company_name" id="company_name">
                                <h3 id="" class="partyName">Opening Stock</h3>
                            ...
                            <v-app id="inspire">
                                ...
                                <div class="show_entries_for_table">
                                    <v-data-table :page="page" :pageCount="numberOfPages" :headers="selectedHeaders"
                                        :items="closing" :options.sync="options" dense :items-per-page="10"
                                        :server-items-length="totalclosing" :loading="loading" class="elevation-1"
                                        ...
                                        <template v-slot:item.stockItemName="{ item }">
                                            <td><router-link :to="{ name: 'items', params: { guid: item.stockItemGuid } }" class="onclick_details">{{ item.stockItemName }}</router-link></td>
                                        </template>
                                        <template v-slot:item.closingBalance="{ item }">
                                            <td>{{ item.closingBalance }}</td>
                                        </template>
                                        <template v-slot:item.closingRate="{ item }">
                                            <td>{{ item.closingRate }}</td>
                                        </template>
                                        <template v-slot:item.closingValue="{ item }">
                                            <td><span class="font_sans">{{ symbol }}</span> {{ item.closingValue }}</td>
                                        </template>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            headers: [
                { text: 'ItemName', value: 'stockItemName', align: 'left', sortable: false },
                { text: 'Quantity', value: 'closingBalance', align: 'left', sortable: false },
                { text: 'Rate', value: 'closingRate', align: 'left', sortable: false },
                { text: 'Value', value: 'closingValue', align: 'left', sortable: false }
            ],
            page: 1,
            totalclosing: 0,
            numberOfPages: 0,
    ...
    watch: {
        ...
        this.closingstock();
       GoogleAnalytics.openingstock_reports_screen_pagination(this.options.page);
      },
      ...
      this.options.page = 1; 
        ...
        GoogleAnalytics.openingstock_reports_screen_Rows_Per_Page(this.options.itemsPerPage);
      }else{
        this.call_function = "1";
      }
      },
        ...
        options: {
            handler() {
                this.closingstock();
                GoogleAnalytics.openingstock_reports_screen_listing();
            },
        },
        deep: true,
    },
    ...
    methods: {
        update() {
            ...
            GoogleAnalytics.openingstock_reports_screen_datepicker(moment(String(this.$refs.picker.start)).format('yyyy-MM-DD'),moment(String(this.$refs.picker.end)).format('yyyy-MM-DD'));
            this.closingstock();
        },
        left_arrow() {
      ...
      GoogleAnalytics.openingstock_reports_screen_datepicker(moment(String(new_date_start_date)).format('yyyy-MM-DD'),moment(String(new_date_end_date)).format('yyyy-MM-DD'));
      this.closingstock();
                    ...
                    if (response.data.message == "AUTHENTICATION_FAILED") {
                        ...
                        if(response.data.status != "0"){
                            for (let [key, value] of Object.entries(response.data.data.list)) {
                                response.data.data.list[key].closingValue = Math.abs(response.data.data.list[key].closingValue);
                            }
