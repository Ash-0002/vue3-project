...
<template>
  <section>
              ...
              <v-app id="inspire">
                    ...
                    <template v-slot:item.action="{ item }">
                      <td align="center">
                        <p class="pdf_a_download">
                          <img
                            src="/assets/images/1.0/png/pdficon.png"
                            class="img-fluid pdficonimagetable"
                          />
                        </p>
                      </td>
                    </template>
                ...
                >
                  <v-data-table
                    ...
                    :items="salecustomer"
                    :options.sync="options"
                    :items-per-page="this.$store.state.Web1_Store.Sales_Bill_itemsPerPage"
                    :server-items-length="totalsalecustomer"
                    :loading="loading"
                    class="elevation-1"
                    must-sort
                  ...
                  >
                    ...
                    <template v-slot:item.action="{ item }">
                      <td align="center">
                        ...
                        <p v-else>
                        <a @click="VoucherPdf(item._id,item.customisEwayStatus, item.customisEInvoiceStatus)" class="pdf_a_download DwonloadPDFImage" 
                        v-b-tooltip.hover.top="'Download PDF'">
                          <img
                            src="/assets/images/1.0/png/pdficon.png"
                            class="img-fluid pdficonimagetable"
    ...
    <b-modal
      id="link_popup_whatsapp"
      class="vch_type_popup_body centered-modal"
      size="md"
      title=""
      v-model="link_popup_whatsappmodel"
      hide-footer
    >
      <div class="modal-content email_modal_content tx-size-sm">
        <div class="modal-body top_modal_body p-0 hg_popup">
          <p>Please Set mobile number in Ledger</p>
        </div>
      </div>
    </b-modal>
  </section>
...
export default {
  ...
  data() {
    ...
    return {
      link_popup_whatsappmodel: false,
      active_el: "Bill",
      active_date: "oney",
      startDate: startDate,
      endDate: endDate,
      ...
      page: this.$store.state.Web1_Store.Sales_Bill_page,
      totalsalecustomer: 0,
      numberOfPages: 0,
      salecustomer: [],
      value: [],
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.sales_bill_screen_pagination(this.options.page);
    },

    "options.itemsPerPage"() {
    
      setTimeout(() => {
      const StartIndex = (this.$store.state.Web1_Store.Sales_Bill_page - 1) * this.$store.state.Web1_Store.Sales_Bill_itemsPerPage
      const endIndex = StartIndex +  this.$store.state.Web1_Store.Sales_Bill_itemsPerPage 
      const Difference = endIndex - this.totalsalecustomer

        if(Difference > this.totalsalecustomer ){
        console.log(this.totalsalecustomer, "hihihihi")
     ...
     this.page= this.$store.state.Web1_Store.Sales_Bill_page
     this.options.page = this.page; 
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.sales_bill_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.sales_bill_listing();
      },
    },
    deep: true,
  },
  ...
  computed:{
    currentPage() {
      return this.$store.state.Web1_Store.Sales_Bill_page;
    },

  },
  methods: {
    freeusergeneratePdf() {
      GoogleAnalytics.sales_bill_screen_viewpdf("viewpdf");
    },
    StoredPage(newValue){
      this.$store.dispatch('Web1_Store/setSalesBillPage', newValue.page)
      

    },
    updateItemsPerPage(newValue){
      this.$store.dispatch('Web1_Store/setSalesBillItemsPerPage', newValue)
    ...
    get_link_url(_id){
      ...
      Api.generateWhatsappCode(data, headers)
          .then((response) => {
        if (response.data.message == "AUTHENTICATION_FAILED") {
          localStorage.clear();
          window.location.href = "/";
          ...
          this.whatapp_link = "";
          this.whatapp_link = "https://lvkp.in/" + response.data.data.activatedCode
        }
      })
      .catch((err) => {
        Common.getStatusForActionFailed(err.response.status);
    ...
    whatapp_send(partyLedgerGuid,voucherNumber,_id){
      const data = {
        EndLimit : 100,
        requestFrom: "WEB",
        company_id: localStorage.getItem("companyId"),
        _userId: localStorage.getItem("customerId"),
        ledgerGuid : partyLedgerGuid,
      ...
      Api.getLedgerByIdGo(data, headers)
        ...
        if (response.data.message == "AUTHENTICATION_FAILED") {
          ...
          if(response.data.data.ledgerMobile != ""){
            this.get_link_url(_id);
            setTimeout(() => {
              var w_url = "https://wa.me/" + response.data.data.ledgerMobile + "/?text=Hi%0APlease%20find%20the%20invoice%28s%29%20in%20the%20link%20below%0AVoucher%20number%28s%29%0A" + voucherNumber +"%0ALink%20- "+ this.whatapp_link +"%20%0A%0AAccounts%20Department%0A" + this.companyName +"%0A%0A-sent%20via%20livekeeping.com";
              window.open(w_url, "_blank");
            }, 1000);
          }else{
            // alert("Please Set mobile number in Ledger");
            this.link_popup_whatsappmodel = true;
          }
        }
      })
      .catch((err) => {
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.sales_bill_screen_date_filter("1M");
        ...
        // one week
        GoogleAnalytics.sales_bill_screen_date_filter("1W");
        ...
        // 3 month
        GoogleAnalytics.sales_bill_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.sales_bill_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.sales_bill_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.sales_bill_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.sales_bill_screen_search(this.search);
    ...
    updateSort(byDesc, event) {
      ...
      this.sale_data("", this.groupSelectValue, this.switch_type,  this.orderBy);
      GoogleAnalytics.sales_bill_screen_sorting(this.sortBy, this.orderBy);
    ...
    update() {
      ...
      GoogleAnalytics.sales_bill_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
...
div#link_popup_whatsapp___BV_modal_outer_ {
    display: flex;
    align-items: center;
    justify-content: center;
}

header#link_popup_whatsapp___BV_modal_header_ {
    background: #fff;
    border-bottom: none;
}

header#link_popup_whatsapp___BV_modal_header_ button.close {
    color: #ffffff;
    position: absolute;
    right: -10px;
    height: 30px;
...
div#link_popup_whatsapp___BV_modal_body_ p {
    margin-bottom: 0;
    background: #ffeeee;
    padding: 10px 15px;
    color: #ed2d2d;
...
div#link_popup_whatsapp .modal-dialog {width: 40%;display: flex;
  }
