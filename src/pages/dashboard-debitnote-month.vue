...
export default {
  ...
  data() {
    ...
    return {
      ...
      headers: [
        ...
        { text: "Amount", value: "totalAmount", align: "right", sortable: true },
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.purchase_month_screen_pagination(this.options.page);
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.purchase_month_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.purchase_month_listing();
      },
    },
    deep: true,
  },
  ...
  methods: {
    ...
    freeusergeneratePdf() {
      GoogleAnalytics.purchase_month_screen_viewpdf("viewpdf");
    },
    VoucherPdf(v_id) {
      const data = {
        requestFrom: "WEB",
    ...
    activate: function (el) {
      ...
      if (el === "Ledger") {
        this.$router.push({ name: "dashboard-debitnote" });
      } else if (el === "Stock Item") {
        this.$router.push({ name: "dashboard-debitnote-stock-item" });
      } else if (el === "Month") {
        this.$router.push({ name: "dashboard-debitnote-month" });
      } else if (el === "Bill") {
        this.$router.push({ name: "dashboard-debitnote-bill" });
      } else if (el === "Ledger Group") {
        this.$router.push({ name: "dashboard-debitnote-ledger-group" });
      } else if (el === "Voucher Type") {
        this.$router.push({ name: "dashboard-debitnote-voucher-type" });
      } else if (el === "Stock Category") {
        this.$router.push({ name: "dashboard-debitnote-stock-category" });
      } else if (el === "Stock Group") {
        this.$router.push({ name: "dashboard-debitnote-stock-group" });
      }
      this.active_el = el;
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.purchase_month_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.purchase_month_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.purchase_month_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.purchase_month_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.purchase_month_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.purchase_month_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.purchase_month_screen_date_filter("Max");
        var new_date_start_date = moment(
          ...
          .then((response) => {
            ...
            if (responseStatus == "200") {
              for (let [key, value] of Object.entries(
                ...
                response.data.data.list[key].ledgerTotalAmount = Math.abs(
                  response.data.data.list[key].ledgerTotalAmount
                );
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.purchase_month_screen_search(this.search);
    ...
    updateSort(byDesc, event) {
      ...
      GoogleAnalytics.purchase_month_screen_sorting(this.sortBy, this.orderBy);
    ...
    update() {
      ...
      GoogleAnalytics.purchase_month_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
