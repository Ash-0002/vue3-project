...
<template>
  <section>
        ...
        <div class="col-lg-6 col-md-6 col-6">
          <div class="title_text" v-if="this.$route.params.voucher_id">
            <div v-if="isTransfer != '0'">
              <h4 class="page-title mb-0">Sales Voucher</h4>
            </div>
            <div v-else>
              <h4 class="page-title mb-0">Update Sales Voucher</h4>
            </div>
          </div>
          <div class="title_text" v-else>
            <h4 class="page-title mb-0">Create Sales Voucher</h4>
          </div>
          <div v-if="dialog" class="modaleWayPopupModal">
      <div class="modal-content-eWayPopup-Content">
        <div class="modal-buttonsThreeeWay">
          <button @click = "OpenEwayForm()">Generate e-Way Bill</button>
          <button @click="generateEinvoice()" >Generate eInvoice</button>
          <button @click = "OpenEwayandEinvoice()">Generate e-Way Bill & eInvoice</button>
        </div>
        <button class="Create-VouchereWayPopup"@click="add_sales">Create Voucher Only</button>
      </div>
    </div>
    <div v-if="this.userName == '' && this.Password == '' && this.ShowLogin" class="modalLogin" id="popup">
        <div class="popup-contenteWayPopup">
            <h2>eInvoice & eWayBill setup</h2>
            <span class="error-validation" v-if="Eway_Error.UpdateGSPError" >{{ Eway_Error.UpdateGSPError }}</span>
            <form>
                <input v-model="this.companyGstNumber"type="text" id="gstNumber" placeholder="GST Number" required>
                
                <div class="info">
                    <p>
                        <span class="info-icon">ℹ️</span>
                        GSP Username and Password is different from e-invoice/ e-way bill system login Username and Password
                    </p>
                </div>
                <div class="inputField">
                <input v-model="usernameValue" type="text" id="gspUsername" placeholder="GSP Username">
            <span class="error-validation" v-if="Login_error.userNameError" >{{ Login_error.userNameError }}</span>

                
                <input v-model="passwordValue" type="text" id="gspPassword" placeholder="GSP Password">
            <span class="error-validation" v-if="Login_error.passwordError" >{{ Login_error.passwordError }}</span>
          </div>

                <button @click.prevent="UpdateGSP()" class="btn-primary">LOGIN</button>
            </form>
            <p class="support">Call support +91 83 83 83 83 83</p>
        </div>
    </div>
    <div v-if="EwayPopup" class="modaleWayPopupModal">
      <div class="container eWayPopupcontainer">
        <div class="card eWayBillPopupCradSet">
      <div v-if="voucher_error" id="tax_div_move_top" class="qrl_error_box">
    <p>{{ voucher_error }}</p>
    </div> 
      <div class="row eWayPopupRow">
        <div class="col eWayPopCol">
          <h3>eWay Bill Details <span class="beta-tag">BETA</span></h3>
        </div>
      </div>
      ...
      <form>
        <div class="row eWayPopupRow">
          <div class="col eWayPopCol">
            <div class="invoice_radio_button InvoiceeWayRadioButton">
              <label class="EWayPopLabel"><b>Supply Type</b></label>
              <input
                class="invoice_radio eWayPopupRadioInput"
                type="radio"
                id="OutwardRadio"
                name="supplyType"
                value="Outward"
                v-model="RadioValue"
              />
              <label for="OutwardRadio" class="EWayPopLabel">Outward</label>
              <input
                class="invoice_radio eWayPopupRadioInput"
                type="radio"
                id="InwardRadio"
                ...
                v-model="RadioValue"
              />
              <label for="InwardRadio" class="EWayPopLabel"> Inward</label>
            </div>
          </div>
        </div>
        <div class="row eWayPopupRow">
          <div class="col-lg-4 col-12 eWayPopCol mb-3">
            <label for="billDate" class="EWayPopLabel">Bill Date</label>
            <div
              class="datepicker_image_set input-disable-overlay-hide-all-div"
            >
          ...
          <div class="col-lg-4 col-12 eWayPopCol mb-3">
            <label for="subSupplyType" class="EWayPopLabel">Sub Supply Type</label>
            <select v-model="SubSupplyTypeValue" id="subSupplyType" name="subSupplyType"  @change = "SupplyTypeChanged($event)">
              <option value="">Enter Supply Type</option>
              <option
                v-for="(Data, index) in subSupplyTypeData"
                :key="index"
          ...
          <div class="col-lg-4 col-12 eWayPopCol mb-3">
            <label for="documentType" class="EWayPopLabel">Document Type</label>
            <select v-model="DocumentTypeValue" id="documentType" name="documentType" @change = "DocumentCodeFunction($event)">
              <option value="">Enter Document Type</option>
              <option
                v-for="(Data, index) in documentTypeData"
                :key="index"
                :value="Data.description"
              >
                {{ Data.description }}
            ...
            <!-- <label for="transporterID">Distance</label>
            <input
              type="text"
             ...
             /> -->
            <span class="error-validation" v-if="Eway_Error.documentTypeData">{{ Eway_Error.documentTypeData }}</span>
          </div>
        </div>
        <div class="divider"></div>
        <div class="row eWayPopupRow mt-3">
          <div class="col eWayPopCol" v-if="isButtonChecked">
            <h3>Transport Details</h3>
            <div class="divider mt-2 mb-2"></div>
            <div class="row">
            <div class="col-lg-6 col-12 mb-2">
              <label for="transporterName" class="EWayPopLabel">Transporter Name</label>
            <input
            type="text"
            ...
            <div class="col-lg-6 col-12 mb-2">
              <label for="transporterID" class="EWayPopLabel">Transporter ID</label>
         <input
        type="text"
        ...
        />
        <span class="error-validation" v-if="Eway_Error.transporterId">{{ Eway_Error.transporterId }}</span>
            </div>
            </div>
            
        <label class="EWayPopLabel"
              ><input
              @change="updateFlag" 
              type="checkbox" name="manualEntry" /> <span class="paddinginputleftside">I want to enter
              details manually</span></label
            >

         </div>
          <div class="col eWayPopCol" v-else>
            <h3>Transport Details</h3>
            <div class="divider mt-2 mb-2"></div>
            <div class="row">
            <div class="col-lg-6 col-12 mb-2">
              <label for="transporterName" class="EWayPopLabel">Transporter Name</label>
            <select v-model="TransporterNameValue"id="transporterName" name="transporterName">
              <option  value="">Select Transporter Name</option>
              <option v-for="(Data , index) in TransporterName" :value="Data.ledgerName"
              :key="index">
                {{ Data.ledgerName }}
              </option>
            </select>
            ...
            <div class="col-lg-6 col-12 mb-2">
              <label for="transporterID" class=EWayPopLabel>Transporter ID</label>
            <select v-model="TransportIdValue" id="transporterID" name="transporterID">
              <option value="" >Select Transporter ID</option>
              <option v-for="(Data, index) in TransporterId"
              :value="Data.transporterId"
              :key="index"
              >{{ Data.transporterId}} </option>
            </select>
            <span class="error-validation" v-if="Eway_Error.transporterId">{{ Eway_Error.transporterId }}</span>
            </div>
            </div>
        ...
        <div class="divider mt-2 mb-2"></div>
        <div class="CloseAndDetails">
        <p class="PartB" @click="ShowPartBFunction()" v-if="this.ShowPartB">
          <i class="fa fa-times"></i> Close</p>
        <p v-else class="PartB PartDetailsContent" @click = "ShowPartBFunction()">
          <i class="fa fa-plus"></i> Add Part B Details</p>
      </div>
        <div v-if="this.ShowPartB" class="row">
          <div class="col eWayPopCol">
            <h3>Part B Details</h3>
            <div class="divider mt-2 mb-2"></div>
            <div class="row eWayPopupRow">
              <div class="col-lg-4 col-12 mb-2">
                <label for="mode" class="EWayPopLabel">Mode</label>
                <select v-model="TransportModeValue" @change = "transportModeCode($event)" id="mode" name="mode">
                  <option value="">Select Mode</option>
                  <option v-for="(Data , index) in TransportMode "  
                      :key="index"
                      :value="Data.description"  selected>{{ Data.description }}</option>
                </select>
                <span class="error-validation" v-if="Eway_Error.TransportMode">{{ Eway_Error.TransportMode }}</span>
              </div>
              <div class="col-lg-4 col-12 mb-2">
                <label for="date" class="EWayPopLabel">Date</label>
                <div class="datepicker_image_set input-disable-overlay-hide-all-div">
                <span class="sales_date_img_new">
                <img
                  src="/assets/images/1.0/sales/calendar.svg"
                  class="img-fluid"
              ...
              <div class="col-lg-4 col-12 mb-2">
                <label v-if="this.TransportModeValue == 'Road'"for="documentNumber" class=EWayPopLabel>Doc/Landing/RR/AirWay No.(Optional)</label>
            <label v-else for="documentNumber" class="EWayPopLabel">Doc/Landing/RR/AirWay No.</label>

            <input v-model="DocumentNumber" type="text" id="documentNumber" name="documentNumber" />
            <span class="error-validation" v-if="Eway_Error.DocumentNumber">{{ Eway_Error.DocumentNumber }}</span>
              </div>
            </div>
            
            <div class="row eWayPopupRow">
              <div class="col-lg-4 col-12 mb-2">
                <label v-if="this.TransportModeValue != 'Road'" for="vehicleNumber" class="EWayPopLabel">Vehicle No (Optional)</label>
                <label v-else for="vehicleNumber" class="EWayPopLabel">Vehicle No</label>
                <input v-model="vehicleNumberValue" type="text" id="vehicleNumber" name="vehicleNumber" />
                <span class="error-validation" v-if="Eway_Error.vehicleNumberValue">{{ Eway_Error.vehicleNumberValue }}</span>
              </div>
              <div class="col-lg-4 col-12 mb-2">
                <label for="vehicleType" class="EWayPopLabel">Vehicle Type</label>
                <select @change = "VehicleTypeModeCode($event)" v-model="VehicleTypeValue" id="vehicleType" name="vehicleType">
                  <option value=""  selected>
                    Select Vehicle Type
                  </option>
                  <option v-for="(Data, index) in VehicleType"
                  :value = "Data.description"
                  :key="index"
        ...
        <div class="row text-center eWayPopupRow">
          <div class="col eWayPopCol">
            <button v-if="this.EwayandEinvoice" @click = "generateEwayandEinvoice()" type="button" class="btn-primary" >Save</button>
            <button v-else @click="generateEway() " type="button" class="btn-primary">SAVE</button>
          </div>
        </div>
      </form>
    </div>
    ...
    <div v-if="Inventory_error" id="tax_div_move_top" class="qrl_error_box">
      ...
      <p v-if="Inventory_error.consignee_pincode">{{ Inventory_error.consignee_pincode }}</p>
      <p v-if="Inventory_error.buyer_pincode">{{ Inventory_error.buyer_pincode }}</p>
      <p v-if="Inventory_error.consignee_gstin">{{ Inventory_error.consignee_gstin }}</p>
      <p v-if="Inventory_error.buyer_gstin">{{ Inventory_error.buyer_gstin }}</p>
      <p v-if="Inventory_error.DispatchName">{{ Inventory_error.DispatchName }}</p>
          ...
          <div class="row">
                ...
                <div class="sales_cstm_dropdown input_light_border w_100_div">
                  ...
                  <form   autocomplete="off" @submit.prevent class="input-disable-overlay-hide-all-div">
                    <Dropdown  :options="partyData" v-on:selected="partySelected" v-on:filter="partyfilter"
                      v-model="party_name" :disabled="isTransfer == '1'" :maxItem="25"
            ...
            <div class="col-lg-6 col-12">
              ...
              <div class="form-group input_new_textbox mb-4">
                <input  @input="inputVoucherNumber" type="text" :disabled="isTransfer == '1'" v-model="voucherNumber" placeholder=""
              ...
              <div class="row">
                  ...
                  <div class="editing_table_for_voucher">
                    <table class="new_voucher_table with_inventory_table">
                      ...
                      <tr class="inventory_table_tr">
                        ...
                        <!-- <td class="inventory_table_th">HSN Code</td> -->
                      ...
                      <tr v-for="(item, index) in tableRows" :key="item.id" class="margin-top-set-for-tax">
                        ...
                        <td width="8%">
                          <input type="number" :disabled="disabled === '1' || isTransfer == '1'" @keydown="handleKeyDown"
                            @keyup="
                               new_rate($event, index), amount_calculation(index)
                        ...
                        <td class="rate_position_relative_innput">
                          <input type="number" @keydown="handleKeyDown" :disabled="disabled === '1' || isTransfer == '1'"
                            @keyup="amount_calculation(index)" v-model="item.rate" placeholder="Rate"
                        ...
                        <td>
                          <input type="number" @keydown="handleKeyDown" :disabled="disabled === '1' || isTransfer == '1'"
                            @keyup="amount_calculation(index)" v-model="item.discount" placeholder="Discount"
                    ...
                    <table class="new_voucher_table">
                      ...
                      <tr v-for="(taxSummary, taxindex) in taxarray" :key="taxindex" class="valign_td_set">
                        <td width="25%">
                          ...
                          <form autocomplete="off" @submit.prevent @focusin="clearSearch">
                            <div v-if="taxSummary.tax_select !== ''" class="input-disable-overlay-hide-all-div">
                              <Dropdown
                                :options="taxData"
                                @selected="tax_value_select($event, taxindex)"
                                @filter="taxfilter"
                                v-model="tax_name"
                                :disabled="isTransfer === '1'"
                                :maxItem="25"
                                :placeholder="taxSummary.tax_select"
                              />
                            </div>
                  ...
                  <div class="panel panel-primary">
                    <div class="tab-menu-heading p-0">
                      <div class="tabs-menu m-0">
                        <ul class="nav nav-tabs nav-tabs-border-none pt-1 sales_voucher_tabs_sec m-0" role="tablist">
                          ...
                          <!-- <li v-if="EwayEinvoiceElligible"class="nav-item">
                            <a class="nav-link" id="DispatchFrom-tab" data-bs-toggle="tab" href="#DispatchFrom" role="tab" aria-controls="DispatchFrom" aria-selected="false">Dispatch From</a>
                          </li> -->
                        </ul>
                      </div>
                    </div>
                    <div class="tab-content main_tab_content sales_voucher_tab_content">
                      <div class="tab-content main_tab_content pb-3">
                        ...
                        <div class="tab-pane fade show active" id="buyerdetails" role="tabpanel" aria-labelledby="buyerdetails-tab">
                          <div class="row">
                            ...
                            <div class="col-lg-4">
                              ...
                              <div class="form-group mb-4">
                                <select name="user[city]" @click="buyer_regisration_type_click()" :disabled="isTransfer == '1'" v-model="buyer_regisration_type"
                        ...
                        <div class="tab-pane fade" id="orderdetails" role="tabpanel" aria-labelledby="orderdetails-tab">
                          <div class="row">
                            ...
                            <div class="col-lg-4">
                              ...
                              <div class="form-group input_new_textbox mb-4">
                                <input  type="text" :disabled="isTransfer == '1'" v-model="order_terms_of_payment"
                        ...
                        <div class="tab-pane fade" id="DispatchFrom" role="tabpanel" aria-labelledby="DispatchFrom-tab">
                          <div class="row">
                            <div class="col-lg-4">
                              <label class="form_input_lbl">Name</label>
                              <div class="form-group">
                               <input v-model="DispatchName" type="text" id="name" placeholder="Name">
                               <span class="error-validation" v-if="Inventory_error.DispatchName" >{{ Inventory_error.DispatchName }}</span>
                            </div>
                            </div>
                            <div class="col-lg-4">
                       <label for="state">State Name</label>
                       <select v-model="DispatchStateName" id="state">
                         <option value="">select</option>
                         <option v-for="(Data, index) in stateData"
                         :key="index"
                         :value="Data.name">{{ Data.name }}</option>
                          </select>
                          <span class="error-validation" v-if="error.DispatchStateName" >{{ error.DispatchStateName }}</span>
                     </div>
                     <div class="col-lg-4">
                          <label for="address">Address</label>
                          <input v-model="DispatchAddress" type="text" id="address" placeholder="Address">
                     </div>
                     <div class="col-lg-4">
                          <label for="place">Place</label>
                          <input v-model="DispatchPlace" type="text" id="place" placeholder="Place">
                        </div>
                        <span class="error-validation" v-if="error.DispatchPlace" >{{ error.DispatchPlace }}</span>
                        <div class="col-lg-4">
                          <label for="pincode">Pincode</label>
                          <input v-model="DispatchPincode" type="text" id="pincode" placeholder="Pincode">
                        </div>
                          </div>
                        </div>
                      </div>
                ...
                <div class="col-12">
                  <div class="addmoreledgers-text">
                    <button :disabled="isTransfer == '1' || add_ledger_button_show == 1" v-on:click="add_ledger_click()">
                      Add More Ledgers
                    </button>
                  </div>
                    ...
                    <div class="editing_table_for_voucher">
                      <table class="new_voucher_table">
                        ...
                        <tr v-for="(ledger, index) in ledger_array" :key="ledger.id">
                          ...
                          <td>
                            <select name="" id="" :disabled="isTransfer == '1'" class="form-select item_select" @change="ledger_amount_keyup()"
                              v-model="ledger.type">
            ...
            <div class="row">
              ...
              <div class="col-lg-6">
                ...
                <div class="form_bottom_button_save" v-else>
                  <!-- <button v-if="EwayEinvoiceElligible" :disabled="disabled === '1'" class="form_button black_button" @click="handleSaveButton()">
                    Save
                  </button> -->
                  <button :disabled="disabled === '1'" class="form_button black_button" @click="add_sales()">Save</button>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      userName:'user',
      Password: 'pass',
      update_eWayBillDetails : [],
      update_isEInvoiceStatus : "",
      update_dispatchFromDetails : [],
      subSupplyTypeData: [],
      documentTypeData: [],
  ...
  computed:{
    EwayEinvoiceElligible(){
      return this.companyGstNumber && this.parentUserId == '' && this.isEwayStatus && ((this.customerStatus == "Installation Done" || this.customerStatus == "Churned") || ((this.customerStatus == "Converted" || this.customerStatus == "Converted Churned")))
    }
  },
  ...
  methods: {
    ...
    UpdateGSP(){
      ...
      .then((response)=>{
        if(response.data.status == "200"){
          ...
          console.log(response.data.data, "hi i am update api")
        }
        else{
    ...
    generateEway(){
      ...
      if(this.TransporterNameValue === ""){
        ...
        if(this.ShowPartB && this.TransportModeValue == ''){
          this.Eway_Error.TransportMode = "Please select Mode"

        }
        if(this.ShowPartB && this.TransportModeValue != 'Road' && this.DocumentNumber == ''){
          this.Eway_Error.DocumentNumber = "Please select Document No."

        }
        if(this.ShowPartB && this.TransportModeValue == 'Road' && this.vehicleNumberValue == ''){
      ...
      if (Object.keys(this.Eway_Error).length === 0) {
        ...
        transDocNo: this.DocumentNumber,
    ...
    getGspSearchFunction(){
      ...
      .then((response)=>{
        // console.log(response.data.data, "gsp search data")
    ...
    getTransporterLedgersFunction(){
      const data = { 
        ...
        Api.getTransporterLedgers(data , headers)
        .then((response)=>{
    ...
    OpenEwayForm() {
      ...
      Api.getMasterCodeDetails(data, headers).then((response) => {
        const responseData = response.data.data.subSupplyType
        this.subSupplyTypeData = responseData.filter(item =>  this.allowed_descriptions.includes(item.description));
    ...
    DocumentCodeFunction(event){
        const Selected = this.documentTypeData.find((data)=> data.description === this.DocumentTypeValue)
      this.selectedDocCode = Selected ? Selected.code : ''
    },
     transportModeCode(event){
        const Selected = this.TransportMode.find((data)=> data.description === this.TransportModeValue)
      this.selectedTranModeCOde = Selected ? Selected.code : ''
    },
     VehicleTypeModeCode(event){
        const Selected = this.VehicleType.find((data)=> data.description === this.VehicleTypeValue)
      this.selectedVehicleTypeCOde = Selected ? Selected.code : ''
    },
    SupplyTypeChanged(event){
      const Selected = this.subSupplyTypeData.find((data)=> data.description === this.SubSupplyTypeValue)
    ...
    handleSaveButton(){
      ...
      if(this.order_no == ""  && this.order_date){
        ...
        if(this.DispatchName == ""){
          this.Inventory_error["DispatchName"] = ""
          this.Inventory_error["DispatchName"] = "Please Enter Dispatch Name."
          this.error["DispatchName"] = "Please Enter Dispatch Name."

        }
        if(this.DispatchStateName == ""){
          this.Inventory_error["DispatchStateName"] = ""
          this.Inventory_error["DispatchStateName"] = "Please Enter Dispatch State."
          this.error["DispatchStateName"] = "Please Enter Dispatch State."

        }
        if(this.DispatchPlace == ""){
          this.Inventory_error["DispatchPlace"] = ""
          this.Inventory_error["DispatchPlace"] = "Please Enter Dispatch Place."
          this.error["DispatchPlace"] = "Please Enter Dispatch Place."
        ...
        if(this.buyer_gstin == ''){
          this.Inventory_error["buyer_gstin"] = ""
          this.Inventory_error["buyer_gstin"] = "Please enter Buyer Gstin."
          this.error["buyer_gstin"] = "Please enter Gstin."

        }
        if(this.consignee_gstin == ''){
          this.Inventory_error["consignee_gstin"] = ""
          this.Inventory_error["consignee_gstin"] = "Please enter Consignee Gstin."
          this.error["consignee_gstin"] = "Please enter Gstin."

        }
        if(this.buyer_place_of_supply == ''){
          this.Inventory_error["buyer_place_of_supply"] = ""
          this.Inventory_error["buyer_place_of_supply"] = "Please fill Place of Supply."
          this.error["buyer_place_of_supply"] = "Please fill Place of Supply."

        }
        if(this.buyer_pincode == ''){
          this.Inventory_error["buyer_pincode"] = ""
          this.Inventory_error["buyer_pincode"] = "Please fill Buyer's Postal Code"
          this.error["buyer_pincode"] = "Please fill Buyer's Postal Code"

        }
        if(this.consignee_pincode == ''){
          this.Inventory_error["consignee_pincode"] = ""
          this.Inventory_error["consignee_pincode"] = "Please fill Consignee Postal Code"
          this.error["consignee_pincode"] = "Please fill Consignee Postal Code"

        }
        if( this.voucherNumber.length > 16){
          this.error['voucherNumber'] = "max limit is 16 digits"
        }
        if( this.voucherNumber == '' || !this.voucherNumberCheck){
          this.error['voucherNumber'] = "Invalid Voucher number(only /,- is allowed, 0 can't be at last)"
        }
    ...
    inputVoucherNumber(){
      ...
      const lastDigitNotZeroRegex = /[^0]$/;
      this.voucherNumberCheck = numericRegex.test(this.voucherNumber) &&
                          containsDigitRegex.test(this.voucherNumber) &&
                          lastDigitNotZeroRegex.test(this.voucherNumber);
      console.log(this.voucherNumberCheck, "voucher number")
    ...
    buyer_regisration_type_click(){
      if(this.buyer_regisration_type != "Unregistered"){
   ...
   disableArrowKeys: function (e) {
    ...
    itemselected(selection, index, voucher_id = "") {
      ...
      this.ItemHsnCode = selection.hsnCode      
      this.SelectedItemGuid = selection.guid
      this.final_ledgerEntries = [];
      this.final_select = [];
  ...
  this.getitemlist();
    ...
    taxfilter(keyword) {
      console.log(this.search, "---tax filter search 1");
      ...
      console.log(this.search, "tax filter search 2");
      this.gettaxlist();
    },
 
    clearSearch() {
      console.log("tax select focus search");
 
      this.search = "";
      this.gettaxlist(); // Show all options again
    },
    ...
    gst_calulation_row_wise() {
      ...
      if (this.isIGST == "yes") {
        ...
        // Only CGST and SGST Calculation
        if (this.tax_igst != 0) {
          this.taxarray_value.push({
            tax_select: "IGST",
            amount: parseFloat(0).toFixed(2),
    ...
    tax_value_per(event, taxindex) {
      ...
      // Validate if the input value is a valid number and not NaN
      if (isNaN(inputValue)) {
      ...
      this.tableRows.forEach((value, key) => {
        if(this.tableRows[index].taxInclusiveChecked == true){
          ...
          var inner_object = {
            ...
            itemRate: isNaN(itemRate) ? "0" : itemRate.toFixed(2).toString(),
                  ...
                  response.data.data.itemGstDetails.forEach((value, key) => {
                    if (response.data.data.igstApplicable == true) {
                      ...
                      this.final_object.push({
                        ...
                        baseUnits : this.tableRows[key].baseUnits,
                        rate:
                        ...
                        ledgerEntry: [],
                        hsnCode: this.tableRows[key].hsn_code,
    ...
    tax_value_select(selectedTax, taxindex = 0, ledgerarray = {}) {
      console.log(selectedTax, "Tax_selected");
    
      if (Object.keys(ledgerarray).length !== 0) {
        var amount_round = parseFloat(this.taxarray[taxindex].amount);
    
        setTimeout(() => {
          console.log(ledgerarray, "this is the ledgerarray value");
        ...
        const taxDetails = selectedTax;
        var amount_round = parseFloat(this.taxarray[taxindex].amount);
    
        this.ledgerTaxType_value = "";
        if (taxDetails.name.toLowerCase().includes("cgst")) {
          this.ledgerTaxType_value = "cgst";
        } else if (taxDetails.name.toLowerCase().includes("sgst")) {
          this.ledgerTaxType_value = "sgst";
        } else if (taxDetails.name.toLowerCase().includes("igst")) {
        ...
        this.final_part_ledgerEntries.push({
          ...
          bankAllocationDetails: [],
          isMoreLedgerField: "1",
          ledgerName: taxDetails.name,
          ledgerGroup: taxDetails.parentgroup,
          ledgerTaxType: this.ledgerTaxType_value,
          ledgerGroupParent: taxDetails.parent,
          ledgerGuid: taxDetails.taxguid,
          masterReservedName: taxDetails.masterReservedName,
          amount: parseFloat(amount_round.toFixed(2)),
        });
    
        this.$set(this.final_ledgerEntries, taxindex, this.final_part_ledgerEntries[0]);
    
        this.tax_name = taxDetails.name;
        this.taxarray[taxindex].tax_select = taxDetails.name;
    
        console.log("Updated taxarray value", this.taxarray);
      }
    },
    v_type_encoded() { 
      // Assuming you have the full encoded URL
      const fullPath = this.$route.fullPath;
      const parsedVoucherType = fullPath.split('/sales-voucher/')[1];
      const decodedVoucherType = decodeURIComponent(parsedVoucherType);
      return decodedVoucherType;
    },
    add_sales() {
      ...
      this.taxarray.forEach((value, key) => {
        if (!value.tax_select) {
          if (!this.error.tax_select) {
            this.error.tax_select = {}; // Create an object to store individual errors for each select field
          }
          this.error.tax_select[key] = "Tax is required"; // Set the error message for the current select field
     
        }
      });
      // if(this.tableRows[0].item_name != "Select Item"){
        this.tableRows.forEach((value, key) => {
      ...
      if (Object.keys(this.error).length === 0) {
        ...
        this.final_ledgerEntries.push({
          ...
          amount: -parseFloat(this.gross_amount).toFixed(2),
        ...
        const data = {
          voucherType: this.v_type_encoded(),
          voucherDate:  moment(String(this.customDate)).format("YYYY-MM-DD"),
    ...
    //         transMode: this.selectedTranModeCOde,
    //         transDocNo: this.DocumentNumber,
    //         transDocDate: moment(String(this.transDocDate)).format("YYYY-MM-DD") ,
    //         vehicleNo: this.vehicleNumberValue,
                  ...
                  if (response.data.data.igstApplicable == false) {
                    ...
                    this.final_ledgerEntries.forEach((value, key) => {
                      if(value.ledgerName == this.taxarray[0].tax_select){
                        this.final_ledgerEntries[key].amount = this.roundToTwoDecimalPlaces((response.data.data.voucherGstDetails.gstAmount / 2));
                      }
    ...
    setItemsData(selection, key, label, preValue) { 
      ...
      if(label === "update") {
        ...
        this.item_gstApplicable = selection.gstAppliacable;
          ...
          this.tableRows[key].rate = parseFloat(preValue.rate.split('/')[0]);
...
,
      ...
      if (selection.standardSellingPriceList.length != "0") {
        ...
        if (applicableDateArray.length > 1) {
          ...
          applicablePriceLevel = applicableDateArray.reduce((min, e) => {
            ...
            const minDate = moment(min.date, "YYYY-MM-DD HH:mm:ss");
            const currentDate = moment(moment(String(this.customDate)).format("yyyy-MM-DD"),
            "YYYY-MM-DD HH:mm:ss");
            return currentDate.isAfter(moment(minDate, "YYYY-MM-DD HH:mm:ss"))
              ? e
              : min;
          });
          console.log("s1", applicableDateArray);
        } else {
          applicablePriceLevel = applicableDateArray[0];
        }
        let priceLevelList = [applicablePriceLevel];
        console.log("kartik=========",priceLevelList);
        let rateValue;
        if (priceLevelList.length == "0") {
          return this.getLastRate(selection, index);
        } else {
        ...
        .then((response) => {
          if (response.data.status == "200") {
            this.invoice_type =
              ...
              this.isEwayStatus = response.data.data.vouchers.isEwayStatus
    ...
    async check_gst_calculation() {
      ...
      if (companyGSTIn !== "") {
        // Check if ledger GST is applicable
  ...
  mounted() {
    ...
    GoogleAnalytics.create_sales_voucher_listing();
...
label.tax_inclusive_checkbox input {
    margin-top: 0 !important;
}
.Create-VouchereWayPopup{
...
.modal-buttonsThreeeWay button:hover {
  background-color: #ddd;
}
.modal-content-eWayPopup-Content {
...
span.paddinginputleftside {
    padding-left: 10px;
}
@media only screen and (max-width:768px){
  .modal-content-eWayPopup-Content{width:95%;}
  .card.eWayBillPopupCradSet{height:670px;overflow-y:auto;}
}
