          ...
          <div class="title_text">
            <div class="title_text" v-if="this.$route.params.voucher_id">
              <div v-if="isTransfer !== '0'">
                <h4 class="page-title mb-0">Journal</h4>
              </div>
              <div v-else>
                <h4 class="page-title mb-0">Update Journal</h4>
              </div>
            </div>
            <div v-else>
              <h4 class="page-title mb-0">Create Journal</h4>
            </div>
          </div>
        </div>
        ...
        <div class="new_party_form">
          <div class="row">
            ...
            <div class="col-lg-6 col-12" style="position: relative;">
              ...
              <div class="form-group input_new_textbox mb-4">
                <input type="text" @input="html_tag_notaccept('voucherNumber', $event)" placeholder=""
                  v-model="voucherNumber" class="form-control only-input-disable-all-hide-overlay" :disabled="true"
                  autocomplete="off" />
                <p class="mb-0 " style="
                  position: absolute; 
                  right: 20px; 
                ...
                <form v-if="isEditing" @submit.prevent>
                  <div class="editVoucherboxRechanges CreateJournalEditPopupSet" v-click-outside="closeEvent">
            ...
            <div class="col-lg-6 col-12">
              ...
              <div class="datepicker_image_set mb-4 input-disable-overlay-hide-all-div">
                ...
                <Datepicker v-model="customDate" format="dd MMM yyyy" placeholder="Date" :disabled="isTransfer === '1'" :disabled-dates="disabledBookDates">
                </Datepicker>
              </div>
            </div>
          </div>
          ...
          <div class="row">
                    ...
                    <table class="new_voucher_table with_inventory_table">
                      <tr>
                        ...
                        <th style="text-align: right">
                          <div
                            v-if="isTransfer == '0' || isTransfer == '' || isTransfer === null || isTransfer === '-1'"
                      ...
                      <tr v-for="(item, index) in tableRows" :key="item.id">
                        <td colspan="3" width="30%">
                          <select name="" id="" class="form-select item_select journal_select" v-model="item.item_type"
                            :disabled="isTransfer === '1'" v-on:keydown="disableArrowKeys">
                          ...
                          <div class="sales_cstm_dropdown journal_drop">
                            <form autocomplete="off" @submit.prevent class="input-disable-overlay-hide-all-div">
                              <Dropdown :options="partyData1" v-on:selected="itemSelected($event, index)"
                                v-on:filter="itemfilter1" v-model="item.item_party" :disabled="isTransfer === '1'"
                            ...
                            <input type="hidden" :disabled="disabled === '1' || isTransfer == '1'"
                              v-model="item.item_id" placeholder=""
                              class="form-control only-input-disable-all-hide-overlay" />
                          </div>
                        </td>
                        <td colspan="3" width="30%">
                          <input type="number" @keydown="handleKeyDown" maxlength="15" placeholder="Amount"
                            v-model="item.amount"
                            class="form-control journal_select only-input-disable-all-hide-overlay"
                            :disabled="isTransfer === '1'" autocomplete="off" v-on:keydown="disableArrowKeys"
                          ...
                          <input type="hidden" v-model="item.ledgerGroup" />
                          <input type="hidden" v-model="item.ledgerGroupParent" />
                          <input type="hidden" v-model="item.ledgerGuid" />
                          <input type="hidden" v-model="item.ledgerName" />
                        </td>
                        <td colspan="1" width="10%" style="text-align: right"
            ...
            <div class="col-12">
              <div class="sales-voucher-textarea">
                <textarea @input="html_tag_notaccept('narration', $event)" id="narration" v-model="narration"
                  name="narration" rows="4" placeholder="Narration" cols="50" class="textarea-disable-not-allowed"
...
export default {
  ...
  data() {
    return {
      ...
      options1: [
        { value: "", text: "Select Party Name" },
        { value: "2", text: "Ambica Traders" },
        { value: "3", text: "Canara Bank" },
        { value: "4", text: "Compuchip Systems" },
        { value: "5", text: "Livekeeping Private Limited" },
        { value: "6", text: "Mahalakshmi Limited" },
      ],
      objectItem: "1",
      item: "",
      stringItem: "",
  ...
  methods: {
    ...
    limitDecimalPlaces(item){
      ...
      const data = item.amount.toString()
    ...
    async InnerpartyDetails(search = '') {
      var self = this;
      this.loading = true;
      this.new_loading = true;
      this.search = "";
    ...
    itemfilter1(keyword) {
      this.search1 = keyword;
      this.InnerpartyDetails(keyword);
    },
    create_journal() {
      //get voucherType
      const typeOfVoucher = localStorage.getItem('voucherType');
      if (this.isWordLimitExceeded) {
        return;
      }
      this.error = [];
      // if (typeOfVoucher !== 'None' && this.voucherNumber == "") {
      ...
      if (Object.keys(this.error).length === 0) {
        ...
        if (this.final_amount != "0") {
          this.error["amount_diff"] =
            "Unable to create journal voucher because total does not match";
          $("html, body").animate(
            {
    ...
    InvoiceConfigurations() {
      ...
      Api.getInvoiceConfigurationsGo(data, headers)
        .then((response) => {
          ...
          if (responseStatus == "200") {
            this.invoice_type = response.data.data.vouchers.voucherNumberConfiguration.journal.isSetDefaultNumber;
            this.customisedHeaders = response.data.data.vouchers.customisedHeaders;
            this.customizeVoucherNumber =
              response.data.data.vouchers.voucherNumberConfiguration.journal.customizeVoucherNumber;
  ...
  mounted() {
    GoogleAnalytics.create_journal_listing();
...
<style scoped>
.CreateJournalEditPopupSet {
  left: 38%;
  top: 70px;
}

.CreateJournalEditPopupSet:before {
  left: 90%;
...
@media only screen and (max-width:1110px){
  .CreateJournalEditPopupSet{left:-30%;}
}
@media only screen and (max-width:991px){
  .CreateJournalEditPopupSet{left:0%;}
}
</style>