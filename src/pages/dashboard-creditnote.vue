...
export default {
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.sales_screen_pagination(this.options.page);
    },
  ...
  methods: {
    freeusergeneratePdf() {
      GoogleAnalytics.sales_screen_viewpdf("viewpdf");
    },
    generatePdf() {
      GoogleAnalytics.sales_screen_viewpdf("viewpdf");
    ...
    updateSort(byDesc, event) {
      ...
      GoogleAnalytics.sales_screen_sorting(this.sortBy, this.orderBy);
    ...
    activatedate: function (date) {
      if (date === "oned") {
        ...
        GoogleAnalytics.sales_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.sales_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.sales_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.sales_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.sales_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.sales_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        Api.getVouchersSummaryByLedgerListGo(data, headers)
          .then((response) => {
            ...
            if (responseStatus == "200") {
              console.log(response.data.data, "ledgerlist response ");
              for (let [key, value] of Object.entries(
                ...
                response.data.data.list[key].amount = Math.abs(response.data.data.list[
                  key
                ].amount)
              }
              ...
              console.log(response.data.data.list, "ledgerlist response ");
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.sales_screen_search(this.search);
    ...
    update() {
      ...
      GoogleAnalytics.sales_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
