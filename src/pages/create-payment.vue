        ...
        <div class="col-lg-6 col-md-6 col-6">
          <div class="title_text" v-if="this.$route.params.voucher_id">
            <div v-if="isTransfer == '0'">
              <h4 class="page-title mb-0">Update Payment</h4>
            </div>
            <div v-else-if="isTransfer == '1'">
              <h4 class="page-title mb-0">Payment Voucher</h4>
            </div>
            <div v-else-if="isTransfer == '-1'">
              <h4 class="page-title mb-0">Update Payment</h4>
            </div>
          ...
          <div class="title_text" v-else>
            <h4 class="page-title mb-0">Create Payment</h4>
          </div>
        </div>
              ...
              <div class="col-lg-6 col-12">
                <div class="form-group input_new_textbox mb-4" style="position: relative;">
                  ...
                  <form v-if="isEditing" @submit.prevent>
                    <div class="editVoucherboxRechanges CreatePaymentEditPopupSet" v-click-outside="closeEvent">
                  ...
                  <div class="sales_cstm_dropdown input_light_border">
                    <form autocomplete="off" @submit.prevent class="input-disable-overlay-hide-all-div">
                      <Dropdown ref="ledgerDropdown" :options="LedgerData" v-on:selected="handleLedgerChange"
                        :disabled="isTransfer === '1'" :maxItem="10" v-model="ledger_name"
                        v-on:filter="ledgerNameFilter" :placeholder="`${this.select_ledgerName}`" autocomplete="off">
                      </Dropdown>
                    </form>
                    <span class="error-validation" v-if="error.ledger_name">{{
                      error.ledger_name
                  ...
                  <table width="100%">
                    ...
                    <tr v-for="(item, index) in receiptAgainstList" :key="index">
                      ...
                      <td width="40%"><span class="font_sans">{{ symbol }}</span> {{
                        Math.abs(parseFloat(item.amount).toFixed(totalDecimalValueForRate)) }}</td>
                      <td align="center" width="5%" v-if="isTransfer === '0' || isTransfer === '-1'"><a
                  ...
                  <div class="form-group input_new_textbox mb-4">
                    ...
                    <div class="sales_cstm_dropdown SelectBankNewSearchanbleInputPortion">
                      <!-- <select name="under" ref="my_input" id="under" class="form-select input_light_border_dropdown"
                        ...
                        <option v-for="(ledger, index) in bankList" :value="ledger.ledgerName" :key="index"
                      ...
                      <input name="under" ref="my_input" id="under" class="form-select input_light_border_dropdown"
                        ...
                        @blur="toggleBankFunction(false)"
                         type="text" placeholder="select bank" @input="filterBanks" />
                ...
                <div class="col-lg-6 col-12">
                  <div class="form-group input_new_textbox mb-4">
                    ...
                    <input type="text"
                      ...
                      @input="validateInstrumentNo" class="form-control only-input-disable-all-hide-overlay" value=""
                      :disabled="isTransfer === '1'" autocomplete="off" />
            ...
            <div class="row">
              <div class="col-lg-6 col-12">
                <div class="form-group input_new_textbox mb-4">
                  ...
                  <div class="sales_cstm_dropdown input_light_border">
                    <form autocomplete="off" @submit.prevent class="input-disable-overlay-hide-all-div">
                      <Dropdown ref="voucherTypeDropdown" :options="voucherTypes" v-on:selected="getNextVoucherNumber"
                        v-on:filter="voucherTypeFilter" v-model="voucherType" :disabled="isTransfer === '1'"
              ...
              <div class="col-lg-6 col-12">
                <div class="form-group input_new_textbox mb-4" style="position: relative;">
                  <div class="label_mb_input">Voucher Number</div>
                  <input @input="html_tag_notaccept('voucherNumber', $event)" type="text" v-model="voucherNumber"
                    :disabled="true" placeholder="" class="form-control only-input-disable-all-hide-overlay"
                    autocomplete="off" />
                  <p class="mb-0 " style="
                      position: absolute; 
                      right: 10px; 
                  ...
                  <form v-if="isEditing" @submit.prevent>
                    <div class="editVoucherboxRechanges CreatePaymentEditPopupSet" v-click-outside="closeEvent">
                      ...
                      <div class="SAveCancelButtonRechanges text-right border-top pt-3">
                        ...
                        <button :disabled="isTransfer == 1" @submit.prevent @click="saveEdit"
                          class="VoucherSaveButtonRechanges">
                          Save
                        </button>
                      </div>
                    </div>
              ...
              <div class="col-lg-6 col-12">
                <div class="form-group mb-4">
                  ...
                  <div class="sales_cstm_dropdown input_light_border">
                    <form autocomplete="off" @submit.prevent class="input-disable-overlay-hide-all-div">
                      <Dropdown ref="ledgerDropdown" :options="LedgerData" v-on:selected="handleLedgerChange"
                        v-on:filter="ledgerNameFilter" v-model="ledger_name" :disabled="isTransfer === '1'"
                        :maxItem="10" placeholder="Select ledger" autocomplete="off">
                      </Dropdown>
                    </form>
                    <span class="error-validation" v-if="error.ledger_name">{{
                      error.ledger_name
                ...
                <div class="col-lg-6 col-12">
                  <div class="form-group input_new_textbox mb-4">
                    ...
                    <input type="number"
                      ...
                      placeholder="Bank Account Number" v-on:wheel="disableMouseWheelScroll"
                      class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off"
                      :disabled="isTransfer === '1'" />
                    <span class="error-validation" v-if="error.bank_account_number">{{
                      error.bank_account_number
            ...
            <tbody>
              <tr v-for="(data, index) in popup_data" :key="data.billRef">
                ...
                <td width="20%" v-if="data.closingBalance < 0">
                  ...
                  <label style="display: none;" :for="'newrefradio' + index"><span class="font_sans">{{ symbol
                      }}</span> {{
...
export default {
  ...
  computed: {
    ...
    absoluteInputAmount() {
      return Math.abs(this.data.inputAmount); // Return the absolute value
    },
    VoucherEditable() {
  ...
  methods: {
    ...
    handleLedgerChange(data) {
      this.ledger_name = data.ledgerName;
    ...
    //       outstandingBill.inputAmount = Math.abs(syncBillDetails.amount) || { text: '' };
    ...
    //       }
    //       console.log(this.popup_data[indexOfOutstandingBill].amount_data);
    //     } else {
    ...
    handleCreateName() {
      var temp = {
        ...
        billType: "On Account",
        amount: parseFloat(((parseFloat(this.amount) + this.sum) - parseFloat(this.crSum))).toFixed(this.totalDecimalValueForRate  ),
        billCreditperiod: '',
    ...
    validateAmount() {
      ...
      for (let i = 0; i < this.popup_data.length; i++) {
        if (this.popup_data[i].closingBalance > 0) {
          ...
          let drnum = parseFloat(this.popup_data[i].inputAmount);
          DrtotalInputAmount += (isNaN(drnum) ? 0 : drnum);
        }
      }
      let currentOnAccount = Number(this.amount) + Number(DrtotalInputAmount);
      // Check if total input amount exceeds the popup amount
    ...
    transaction_type_change(event, transaction_type = "") {
      ...
      // this.$refs.ledgerDropdown1._props.placeholder = 'select ledger';
      this.$refs.ledgerDropdown.searchFilter = '';
      this.$refs.ledgerDropdown.selected = '';
      this.select_ledgerName = 'select ledger'
      ...
      return new Promise((resolve, reject) => {
        ...
        Api.getLedgerByIdGo(data, headers)
          .then((response) => {
            ...
            let temp = {};
            if (responseStatus == "200") {
              this.billAllocationdetails = []; // clear it before pushing

              if (this.receiptAgainstList.length || this.receiptCrList.length) {
                this.receiptAgainstList.forEach(item => {
                  temp = {
                    billDate: moment(item.dueDate).format("yyyy-MM-DD"),
                    billType: item.reference.includes('agsref') ? 'Agst Ref' : 'New Ref',
                    amount: parseFloat(item.amount),
                    billCreditperiod: '',
                    billName: item.billRef,
                    billId: ''
                  };
                  this.billAllocationdetails.push(temp);
                });

                this.receiptCrList.forEach(item => {
                  temp = {
                    billDate: moment(item.dueDate).format("yyyy-MM-DD"),
                    billType: item.reference.includes('agsref') ? 'Agst Ref' : 'New Ref',
                    amount: parseFloat("-" + item.amount),
                ...
                temp = {
                  billDate: moment(this.customDate).format("yyyy-MM-DD"),
                  billType: "On Account",
                  amount: Math.round(parseFloat("-" + this.amount).toFixed(this.totalDecimalValueForRate)),
                  billCreditperiod: '',
                  billName: '',
                  billId: ''
                };
    ...
    amountChange(event, amount = "") {
      if (amount !== "") {
        ...
        this.amount = parseFloat(event.target.value).toFixed(this.totalDecimalValueForRate);
      }
    ...
    checkBillUpdateCountCount() {
      ...
      if (this.receiptAgainstList.length != 0 || this.receiptCrList.length != 0) {
        ...
        if (onAccountValid > 0) {
          this.onAccountMsg = `Bill amount  is less than Payment amount (${this.symbol} ${this.amount}) so the remaining amount (${this.symbol} ${(parseFloat(this.amount) - parseFloat(this.crSum)) + this.sum}) will be added as On Account`;
    ...
    addPayment(e) {
      ...
      if (this.ledger_name == "" || this.ledger_name == undefined) {
        this.error["ledger_name"] = "Please select ledger type";
      }
          ...
          .then(() => {
            ...
            Api.addMyVoucher(data, headers)
              ...
              .catch((error) => {
                ...
                this.voucher_error = "Failed to add voucher.";
              });
          })
          .catch((error) => {
            console.error("Ledger fetch failed:", error);
            this.voucher_error = "Unable to fetch party ledger.";
          });
      }
    },
    ...
    get_receipt_data() {
      ...
      Api.getVoucherFullDetailsFromSyncGoo(data, headers)
        .then((response) => {
          ...
          console.log('ledger name in update page', this.ledger_name)
          this.select_ledgerName = this.ledger_name;
          ...
          //calling getMyLedger voucher api
          if (this.ledgerGuid == "") {
    ...
    updatePayment(e) {
      ...
      if (this.transactionseletedname == "Cheque/DD") {
        ...
        console.log(this.bank_name, 'bank =========')
        if (this.bank_name == "") {
          this.error["bank_name"] = "Please select Bank name";
        }
      ...
      if (Object.keys(this.error).length === 0) {
        this.ledgerEntries = [];
        console.log(this.ledgerEntries, "update is working")
        Promise.all([this.getotherLedger(), this.getpartyLedger()])
          .then(() => {
            const data = {
    ...
    InvoiceConfigurations() {
      ...
      Api.getInvoiceConfigurationsGo(data, headers)
        .then((response) => {
          ...
          if (responseStatus == "200") {
            this.invoice_type = response.data.data.vouchers.voucherNumberConfiguration.payment.isSetDefaultNumber;
            this.customisedHeaders = response.data.data.vouchers.customisedHeaders;
            this.customizeVoucherNumber =
              response.data.data.vouchers.voucherNumberConfiguration.payment.customizeVoucherNumber;
  ...
  mounted() {
    ...
    GoogleAnalytics.create_payment_listing();
...
.CreatePaymentEditPopupSet {
  left: 38%;
  top: 70px;
}

.CreatePaymentEditPopupSet:before {
  left: 90%;
...
@media only screen and (max-width:1110px){
  .CreatePaymentEditPopupSet{left:-30%;}
}
@media only screen and (max-width:991px){
  .CreatePaymentEditPopupSet{left:0%;}
}
</style>
