      ...
      <div class="row align-items-center">
        <div class="col-xl-3 col-lg-12 col-12">
          <div class="company_name sales_c_name mobile_padding_left_0 company_heading_title">
            ...
            <h3 id="" class="partyName">My eWay Bills</h3>
          </div>
        </div>
          ...
          <div class="col-12">
            <div class="custom_table">
              <v-app id="inspire">
                ...
                <div
                  class="show_entries_for_table new_show_entries_for_table my-voucher-table-overflow-visible"
                >
                  <v-data-table
                    :page="page"
                  ...
                  >
                    ...
                    <template v-slot:item.expiryDate="{ item }">
                      <td v-if="item.expiryDate != ''">
                        <p>{{ format_date_ex(item.expiryDate) }}</p>
                      </td>
                      <td v-else></td>
                    </template>
                      ...
                      <td align="center">
                        ...
                        >
                          <a
                            ...
                            class="pdf_a_download DwonloadPDFImage"
                          >
                            <img
  ...
  data() {
    ...
    return {
      ...
      headers: [
        ...
        { text: "eWay Status", value: "status_name", sortable: false },
        { text: "Download", value: "pdf", align: "center", sortable: false },
      ],
  ...
  methods: {
    pdfNavigator(data) {
      if (this.parentUserId == "") {
        if (data.isTransfer == "3") {
          return;
        }
        if (data.guid === "") {
          this.eWayBillPdfNoGuid(data._id);
        } else {
          this.eWayBillPdf(data.voucherId);
        }
      } else if (this.parentUserId && this.ewayReadPermission) {
        if (data.isTransfer == "3") {
          return;
        }
        if (data.guid === "") {
    ...
    eWayBillPdfNoGuid(v_id) {
      ...
      const data = {
        ...
        _userId: localStorage.getItem("customerId"),
        requestFrom: "WEB",
        isEway: this.isEwayCurrentStatus,
      };
    ...
    readDataFromAPI(
      ...
      Api.getMyEwayVouchers(data, headers)
        .then((response) => {
          ...
          for (let [key, value] of Object.entries(response.data.data.list)) {
            ...
            if (response.data.data.list[key].isEwayStatus === 1) {
              if (
                response.data.data.list[key].isEwayStatus == 1 ||
                response.data.data.list[key].eWayBillDetails[0]
                  .validUptoWithTime != "" ||
                response.data.data.list[key].eWayBillDetails[0]
                  .billDateWithTime != "" ||
                response.data.data.list[key].eWayBillDetails[0].billDate != ""
              ) {
                if (
                  response.data.data.list[key].eWayBillDetails[0]
                    .validUptoWithTime &&
                  ...
                  response.data.data.list[key].expiryDate =
                  ...
                  // response.data.data.list[key].expiryDate = moment(response.data.data.list[key].expiryDate).subtract(1, 'days').toDate();
                } else if (
                  response.data.data.list[key].eWayBillDetails[0]
                    .billDateWithTime &&
                  response.data.data.list[key].eWayBillDetails[0]
            ...
            // if ((response.data.data.list[key].isEwayStatus !== -1 && response.data.data.list[key].isEwayStatus !== -2) && ((response.data.data.list[key].irnAckDateWithTime && response.data.data.list[key].irnAckDateWithTime !== '') || (response.data.data.list[key].irnAckDate && response.data.data.list[key].irnAckDate !== ''))) {
            //   if (bill.irnAckDateWithTime && bill.irnAckDateWithTime !== '') {
            //     expiryDate = this.convertDateFormat(
            //                   'YYYY-MM-DDTHH:mmZ',
            //                   'YYYY-MM-DD HH:mm',
            //                   bill.irnAckDateWithTime
            //                 );
            //   }else if (bill.irnAckDate && bill.irnAckDate !== '') {
            //     expiryDate = this.convertDateFormat(
            //       'YYYYMMDD',
            //       'YYYY-MM-DD',
            //       bill.irnAckDate
            //     );
            //     expiryDate = moment(expiryDate).add(24, 'hours').toDate();
            //   }
            //   response.data.data.list[key].expiryDate = expiryDate;
            // }

            if (response.data.data.list[key].isEwayStatus == 1) {
              response.data.data.list[key].status_name = "Generated";
              response.data.data.list[key].status_class = "status-generated";
            } else if (response.data.data.list[key].isEwayStatus == -2) {
              response.data.data.list[key].status_name = "Cancelled";
              response.data.data.list[key].status_class = "status-cancelled";
            } else if (response.data.data.list[key].isEwayStatus == -1) {
    ...
    convertDateFormat(currentFormat, convertedFormat, date) {
      return moment(date, currentFormat).utc().format(convertedFormat);
    },
    format_date_ex(value) {
      if (value) {
