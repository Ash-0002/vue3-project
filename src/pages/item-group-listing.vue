    ...
    <div class="page-header-title-box bg-white-all-pages">
      <div class="row align-items-center">
        <div class="col-lg-12 col-12">
          ...
          >
            <h3>{{ handleSymbol(this.$route.params.group_name) }}</h3>
            <div class="company_balance sales_cb">
              <p>Closing Value: <strong>{{ this.totalClosingValue }}</strong></p>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  watch: {
    page: function () {
      ...
      this.groupAPI();
      GoogleAnalytics.item_group_listing_screen_pagination(this.options.page);
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.item_group_listing_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        // this.groupAPI();
        GoogleAnalytics.item_group_listing_screen();
      },
    },
    deep: true,
  },
  methods: {
    ...
    generatePdf() {
      ...
      const data = {
        ...
        groupBy: "Summary",
        voucherType : "Sales",
        parentGuid: this.$route.params.guid,
        isForPdf : true,
        searchTerm: "",
      };
      const headers = {
      ...
      Api.getStockItemsGo(data, headers)
        .then((response) => {
          if (response.data.message == "AUTHENTICATION_FAILED") {
            ...
            this.totalClosingValue = response.data.data.totalClosingValue;
          }
        })
    ...
    updateSortGroup(byDesc, event) {
      ...
      this.groupAPI();
      GoogleAnalytics.item_group_listing_screen_sorting(
        this.sortByGroup,
        this.orderByGroup
      );
    },
    handleBlur() {
      const type = "search";
      GoogleAnalytics.item_group_listing_screen_search(this.search);
      this.groupAPI(type);
    },
  },
  mounted() {
