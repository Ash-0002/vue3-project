              ...
              <v-app id="inspire">
                ...
                >
                  <v-data-table
                    ...
                    :items="salecustomer"
                    :options.sync="options"
                    :items-per-page="this.$store.state.Web1_Store.Sales_SalesOrder_Bill_itemsPerPage"
                    :server-items-length="totalsalecustomer"
                    :loading="loading"
                    class="elevation-1"
                    must-sort
...
export default {
  ...
  data() {
    ...
    return {
      ...
      page: this.$store.state.Web1_Store.Sales_SalesOrder_Bill_page,
      totalsalecustomer: 0,
      numberOfPages: 0,
      salecustomer: [],
      value: [],
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.salesorder_bill_screen_pagination(this.options.page);
    },

    "options.itemsPerPage"() {
 
      setTimeout(() => {
      const StartIndex = (this.$store.state.Web1_Store.Sales_SalesOrder_Bill_page - 1) * this.$store.state.Web1_Store.Sales_SalesOrder_Bill_itemsPerPage
      const endIndex = StartIndex +  this.$store.state.Web1_Store.Sales_SalesOrder_Bill_itemsPerPage 
     ...
     this.page= this.$store.state.Web1_Store.Sales_SalesOrder_Bill_page
     this.options.page = this.page; 
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.salesorder_bill_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.salesorder_bill_listing();
      },
    },
    deep: true,
  },
  ...
  computed:{
    currentPage() {
      return this.$store.state.Web1_Store.Sales_SalesOrder_Bill_page;
    },

  },
  methods: {
    freeusergeneratePdf() {
      GoogleAnalytics.salesorder_bill_screen_viewpdf("viewpdf");
    },
    StoredPage(newValue){
      this.$store.dispatch('Web1_Store/setSalesSalesOrderBillPage', newValue.page)
      

    },
    updateItemsPerPage(newValue){
      this.$store.dispatch('Web1_Store/etSalesSalesOrderBillItemsPerPage', newValue)
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.salesorder_bill_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.salesorder_bill_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.salesorder_bill_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.salesorder_bill_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.salesorder_bill_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.salesorder_bill_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.salesorder_bill_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.salesorder_bill_screen_search(this.search);
    ...
    updateSort(byDesc, event) {
      ...
      GoogleAnalytics.salesorder_bill_screen_sorting(this.sortBy, this.orderBy);
    ...
    update() {
      ...
      GoogleAnalytics.salesorder_bill_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
