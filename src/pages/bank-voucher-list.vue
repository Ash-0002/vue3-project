        ...
        <div class="col-xl-3 col-lg-12 col-12">
          <div class="company_name sales_c_name mobile_padding_left_0">
            <div class="company_balance sales_cb new_title_sales_cb">
              ...
              <div
                class="company_name sales_c_name padding_left_30"
                id="company_name"
              >
                <h3 id="" class="partyName">Bank</h3>
                <div class="company_balance sales_cb">
                  <p>
                    Total Amount:
                    <strong
                      ><span class="font_sans">{{ symbol }}</span>
                      {{ this.grossTotal | formatNumberParties }}</strong
                    >
                  </p>
                </div>
              </div>
              ...
              <v-app id="inspire">
                ...
                <div class="show_entries_for_table">
                  <v-data-table
                    ...
                    :server-items-length="totalledger"
                    :loading="loading"
                    class="elevation-1"
                    must-sort
                    :sort-by="null"
                  ...
                  >
                    ...
                    <template v-slot:item.ledgerName="{ item }">
                      ...
                      <div v-else>
                        <router-link
                          ...
                          :to="{ name: 'Parties', params: { guid: item.ledgerGuid } }"
                    ...
                    <template v-slot:item.balance="{ item }">
                      <td align="right">
                        <p>
                          <span class="font_sans">{{ symbol }}</span>
                          {{ Math.abs(item.balance) | formatNumberParties }}
                          <span v-if = "item.balance < 0">Dr</span>
                              <span v-else-if="item.balance > 0">Cr</span>
                              <span v-else></span>
                        </p>
                      </td>
                    </template>
...
export default {
  ...
  watch: {
    page: function () {
      ...
      this.voucher_list();
      GoogleAnalytics.bank_voucher_list_screen_pagination(this.options.page);
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.bank_voucher_list_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.bank_voucher_list_listing();
      },
    },
    deep: true,
  },
  ...
  methods: {
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.bank_voucher_list_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.bank_voucher_list_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.bank_voucher_list_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.bank_voucher_list_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.bank_voucher_list_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.bank_voucher_list_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.bank_voucher_list_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    update() {
      ...
      GoogleAnalytics.bank_voucher_list_screen_date_filter("Custom");
      this.voucher_list();
      this.date_setting();
    },
    left_arrow() {
        ...
        .then((response) => {
          ...
          if (responseData == "200") {
            for (let [key, value] of Object.entries(response.data.data.list)) {
              response.data.data.list[key].balance = response.data.data.list[key].balance;
            }
            ...
            this.grossTotal = Math.abs(response.data.data.totalBalanceSum);
          } else {
            this.loading = false;
            this.new_loading = false;
            this.ledger = [];
    ...
    updateSort(byDesc, event) {
      ...
      this.voucher_list();
      GoogleAnalytics.bank_voucher_list_screen_sorting(
        this.sortBy,
        this.orderBy
      );
