                    ...
                    <div class="col-12">
                        <div class="custom_table">
                            <v-app id="inspire">
                                ...
                                <v-data-table :page="currentPage" :pageCount="numberOfPages" :headers="selected_stock_item_headers"
                                        :items="salecustomer" :options.sync="options"
                                          :items-per-page="this.$store.state.Web1_Store.StockCategory_itemsPerPage"
                                        ...
                                        ref="table" :hide-default-footer= "true" :footer-props="{'items-per-page-options':[10,25,50,100],'items-per-page-text':'products per page'}"  @pagination= "StoredPage"
                                        ...
                                        <template v-slot:item.name="{ item }">
                                            <td><router-link :to="`${item.url}`" class="onclick_details">{{ item.stockItemName }}</router-link></td>
                                        </template>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            stock_item_headers: [
                ...
                { text: 'Qty', value: 'totalQty', align: 'left' },
                { text: 'Amount', value: 'totalAmount', align: 'right' },
              
            ],
            page: this.$store.state.Web1_Store.StockCategory_page,
            totalsalecustomer: 0,
            numberOfPages: 0,
            salecustomer: [],
            value: [],
    ...
    watch: {
        page: function () {
            ...
            this.sale_data();
            GoogleAnalytics.stock_category_item_screen_pagination(this.options.page);
        ...
        "options.itemsPerPage"() {
            ...
            setTimeout(() => {
            const StartIndex = (this.$store.state.Web1_Store.StockCategory_page - 1) * this.$store.state.Web1_Store.StockCategory_itemsPerPage
      const endIndex = StartIndex +  this.$store.state.Web1_Store.StockCategory_itemsPerPage 
     ...
     this.page= this.$store.state.Web1_Store.StockCategory_page
     this.options.page = this.page; 
        ...
        GoogleAnalytics.stock_category_item_screen_Rows_Per_Page(this.options.itemsPerPage);
      }else{
        this.call_function = "1";
      }
        },
        ...
        options: {
            handler() {
                GoogleAnalytics.stock_category_item_listing();
                this.sale_data();
            },
        },
        deep: true,
    ...
    computed:{
    currentPage() {
      return this.$store.state.Web1_Store.StockCategory_page;
    },

  },
    methods: {
        StoredPage(newValue){
      this.$store.dispatch('Web1_Store/setStockCategoryPage', newValue.page)
      

    },
    updateItemsPerPage(newValue){
      this.$store.dispatch('Web1_Store/setStockCategoryItemsPerPage', newValue)
    ...
    this.$router.go(-1)
    this.$store.dispatch('Web1_Store/resetStockCategoryState');
},
        activatedate:function(date){
        if(date === "oned"){
            // one day
            GoogleAnalytics.stock_category_item_screen_date_filter("1D");
            ...
            // one week
            GoogleAnalytics.stock_category_item_screen_date_filter("1W");
            ...
            // one month
            GoogleAnalytics.stock_category_item_screen_date_filter("1M");
            ...
            // 3 month 
            GoogleAnalytics.stock_category_item_screen_date_filter("3M");
            ...
            // 1 Year
            GoogleAnalytics.stock_category_item_screen_date_filter("1Y");
            ...
            // 2 Year
            GoogleAnalytics.stock_category_item_screen_date_filter("2Y");
            ...
            // this.$refs.picker.end = new_date_end_date;
                ...
                // max date
                GoogleAnalytics.stock_category_item_screen_date_filter("Max");
    ...
    date_setting(){
        ...
        update() {
          ...
          GoogleAnalytics.stock_category_item_screen_date_filter("Custom");
        ...
        sale_data(type,group_name="Stock Item",switch_type ="Gross") {
            ...
            if(group_name === "Stock Item"){
                ...
                Api.getItemsByStockCategoryIdGo(data, headers)
                .then((response) => {
                    if (response.data.message == "AUTHENTICATION_FAILED") {
                        ...
                        for (let [key, value] of Object.entries(response.data.data.list)) {
                            response.data.data.list[key].url = '/items/' + response.data.data.list[key].guid;
                        ...
                        this.totalAmount = Math.abs(response.data.data.grandTotalAmount);
        ...
        switch_change(event){
            ...
            GoogleAnalytics.stock_category_item_screen_filter(event.target.value);
        },
        handleBlur() {
            const type = "search";
            GoogleAnalytics.stock_category_item_screen_search(this.search);
        ...
        updateSort(byDesc, event) {        
            ...
            GoogleAnalytics.stock_category_item_screen_sorting(this.sortBy,this.orderBy);
