        ...
        <div class="col-lg-6 col-md-6 col-6">
          <div class="title_text" v-if="this.$route.params.voucher_id">
            <div v-if="isTransfer !== '0'">
              <h4 class="page-title mb-0">Debit Note</h4>
            </div>
            <div v-else>
              <h4 class="page-title mb-0">Update Debit Note</h4>
            </div>
          </div>
          ...
          <div class="row">
              ...
              <div class="form-group mb-4">
                ...
                <select name="ledger" ref="my_input" id="under" @change="ledgerChange($event)"
                  ...
                  <option v-for="(ledger, index) in ledgerData" :value="ledger.ledgerName" :key="index"
                    :data-id="ledger._id" :data-ledgerguid="ledger.guid">
                    {{ ledger.ledgerName }}
                  </option>
                </select>
            ...
            <div class="col-lg-6 col-12">
              ...
              <div class="form-group input_new_textbox mb-4">
                <input type="text" @input="html_tag_notaccept('voucherNumber', $event)"  :disabled="isTransfer == '1'" v-model="voucherNumber" placeholder=""
                  class="form-control only-input-disable-all-hide-overlay" autocomplete="off" />
            ...
            <div class="col-lg-6 col-12">
              <label class="form_input_lbl">Transfer Return Number</label>
              <div class="form-group input_new_textbox mb-4">
                <input type="text" @input="html_tag_notaccept('transfer_return_number', $event)" :disabled="isTransfer == '1'" v-model="transfer_return_number"
                  placeholder="Transfer Return Number" class="form-control only-input-disable-all-hide-overlay" autocomplete="off" />
              </div>
            </div>
            <div class="col-lg-6 col-12">
              <label class="form_input_lbl">Transfer Date</label>
              <div class="datepicker_image_set mb-4 input-disable-overlay-hide-all-div">
                <span class="sales_date_img_new">
                  <img src="/assets/images/1.0/sales/calendar.svg" class="img-fluid" />
                </span>
                <Datepicker v-model="transfer_date" :disabled="isTransfer == '1'" format="dd MMM yyyy"
                  placeholder="Transfer Date"></Datepicker>
              </div>
            </div>
            ...
            <div class="col-lg-12 col-12">
              <div class="row">
                      ...
                      <tr v-for="(item, index) in tableRows" :key="item.id">
                              ...
                              <div v-if="item.item_name !== undefined" class="input-disable-overlay-hide-all-div">
                                <Dropdown :options="itemsoption" v-on:selected="itemselected($event, index)"
                                  v-on:filter="itemfilter" :disabled="disabled === '1' || isTransfer == '1'
                                    " :maxItem="10" :placeholder="`${item.item_name}`" v-model="item.item_name">
                                </Dropdown>
                              </div>
                              <div v-else class="input-disable-overlay-hide-all-div">
                                <Dropdown :options="itemsoption" v-on:selected="itemselected($event, index)"
                                  v-on:filter="itemfilter" :disabled="disabled === '1' || isTransfer == '1'
                                    " :maxItem="10" placeholder="Select Item" v-model="item.item_name">
                                </Dropdown>
                              </div>
                            </form>
                          </div>
                        ...
                        <td width="8%">
                          <input type="number" @keydown="handleKeyDown" :disabled="disabled === '1' || isTransfer == '1'"
                            @keyup="amount_calculation(index)" v-model="item.quantity" value="1" placeholder="Quantity"
                            class="form-control qty_demo only-input-disable-all-hide-overlay" autocomplete="off" v-on:keydown="disableArrowKeys" v-on:wheel="disableMouseWheelScroll" />
                        </td>
                        <td>
                          <input type="number" @keydown="handleKeyDown" :disabled="disabled === '1' || isTransfer == '1'"
                            @keyup="amount_calculation(index)" v-model="item.rate" placeholder="Rate" class="form-control only-input-disable-all-hide-overlay"
                            autocomplete="off" v-on:keydown="disableArrowKeys" v-on:wheel="disableMouseWheelScroll" />
                        </td>

                        <td>
                          <input type="number" @keydown="handleKeyDown" :disabled="disabled === '1' || isTransfer == '1'"
                            @keyup="amount_calculation(index)" v-model="item.discount" placeholder="Discount"
                            class="form-control only-input-disable-all-hide-overlay" @input="checkDiscount(index)" autocomplete="off" v-on:keydown="disableArrowKeys" v-on:wheel="disableMouseWheelScroll" />
                        </td>
                        <td width="18%">
                          <select :disabled="disabled === '1' || isTransfer == '1'" name="" id=""
                            ...
                            v-model="item.location" @change="amount_calculation(index)">
                        ...
                        <td>
                          <input type="text" :disabled="disabled === '1' || isTransfer == '1'"
                            placeholder="Item Description" class="form-control only-input-disable-all-hide-overlay" @keyup="amount_calculation(index)"
                            v-model="item.item_description" autocomplete="off" />
                        </td>

                     

                        <td>
                          <input type="text" readonly :disabled="disabled === '1' || isTransfer == '1'"
                            placeholder="amount" v-model="item.amount" class="form-control only-input-disable-all-hide-overlay" autocomplete="off" />
                        </td>
                        <td style="text-align: right" @click="isTransfer == '0' && DeleteRow(item.id, index)">
                  ...
                  <div class="editing_table_for_voucher" id="tax_div_move">
                    <table class="new_voucher_table">
                      ...
                      <tr v-if="taxarray != ''" class="inventory_table_tr">
                        ...
                        <td width="25%">
                          <input type="text" @input="html_tag_notaccept('taxSummary', $event)" :disabled="isTransfer == '1'" readonly :v-model="taxSummary.amount"
                            v-bind:value="`${taxSummary.amount}`" placeholder="Amount" class="form-control only-input-disable-all-hide-overlay" />
                        </td>
                    ...
                    <div class="tab-menu-heading p-0">
                      <div class="tabs-menu m-0">
                        <ul class="nav nav-tabs nav-tabs-border-none pt-1 sales_voucher_tabs_sec m-0" role="tablist">
                          ...
                          <li class="nav-item">
                            <a class="nav-link active" id="buyerdetails-tab" data-bs-toggle="tab" href="#buyerdetails" role="tab" aria-controls="buyerdetails" aria-selected="true">Buyer
                              Details</a>
                          </li>
                          <li class="nav-item">
                      ...
                      <div class="tab-content main_tab_content pb-3">
                            ...
                            <div class="col-lg-4">
                              ...
                              <div class="form-group input_new_textbox mb-4">
                                <input type="text" @input="html_tag_notaccept('buyer_place_of_supply', $event)" :disabled="isTransfer == '1'" placeholder="Place of Supply"
                                  v-model="buyer_place_of_supply" class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off" />
                              </div>
                            </div>
                            <div class="col-lg-4">
                              <label class="form_input_lbl">GSTIN / UIN</label>
                              <div class="form-group input_new_textbox mb-4">
                                <input type="text" @input="validateGSTIN" :disabled="isTransfer == '1'" placeholder="GSTIN / UIN"
                                  v-model="buyer_gstin" class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off" />
                        ...
                        <div class="tab-pane fade" id="consigneedetails" role="tabpanel" aria-labelledby="consigneedetails-tab">
                          <div class="row">
                            ...
                            <div class="col-lg-4">
                              ...
                              <div class="form-group input_new_textbox mb-4">
                                <input type="text" @input="validateGSTIN" :disabled="isTransfer == '1'" placeholder="GSTIN / UIN"
                                  v-model="consignee_gstin" class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off" />
                            ...
                            <div class="col-lg-4">
                              ...
                              <div class="form-group input_new_textbox mb-4">
                                <input type="text" @input="html_tag_notaccept('dispatch_delivery_note', $event)" :disabled="isTransfer == '1'" v-model="dispatch_delivery_note"
                                  placeholder="Delivery Note" class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off" />
                              </div>
                            </div>
                            <div class="col-lg-4">
                              ...
                              <div class="form-group input_new_textbox mb-4">
                                <input type="text" @input="html_tag_notaccept('dispatch_doc_no', $event)" :disabled="isTransfer == '1'" v-model="dispatch_doc_no"
                                  placeholder="Dispatch Doc No" class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off" />
                              </div>
                            </div>
                            <div class="col-lg-4">
                              ...
                              <div class="form-group input_new_textbox mb-4">
                                <input type="text" @input="html_tag_notaccept('dispatch_despatch_through', $event)" :disabled="isTransfer == '1'" v-model="dispatch_despatch_through"
                                  placeholder="Dispatch Through" class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off" />
                              </div>
                            </div>
                            <div class="col-lg-4">
                              ...
                              <div class="form-group input_new_textbox mb-4">
                                <input type="text" @input="html_tag_notaccept('dispatch_destination', $event)" :disabled="isTransfer == '1'" v-model="dispatch_destination"
                                  placeholder="Destination" class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off" />
                              </div>
                            </div>
                            <div class="col-lg-4">
                            ...
                            <div class="col-lg-4">
                              ...
                              <div class="form-group input_new_textbox mb-4">
                                <input type="text" @input="html_tag_notaccept('dispatch_motor_vehicle_no', $event)" :disabled="isTransfer == '1'" v-model="dispatch_motor_vehicle_no"
                                  placeholder="Motor Vehicle No" class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off" />
                              </div>
                            </div>
                          </div>
                        </div>
                        <div class="tab-pane fade" id="orderdetails" role="tabpanel" aria-labelledby="orderdetails-tab">
                          <div class="row">
                            ...
                            <div class="col-lg-4">
                              <label class="form_input_lbl">Invoice Date</label>
                              <div class="datepicker_image_set mb-4 input-disable-overlay-hide-all-div">
                                <span class="sales_date_img_new">
                                  <img src="/assets/images/1.0/sales/calendar.svg" class="img-fluid" />
                                </span>
                                <Datepicker :disabled="isTransfer == '1'" v-model="order_invoice_date"
                                  format="dd MMM yyyy" placeholder="Invoice Date">
                                </Datepicker>
                              </div>
                            </div>
                            <div class="col-lg-4">
                              ...
                              <div class="form-group input_new_textbox mb-4">
                                <input type="text" @input="html_tag_notaccept('order_no', $event)" :disabled="isTransfer == '1'" v-model="order_no"
                                  placeholder="Order No" class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off" />
                              </div>
                            </div>
                            <div class="col-lg-4">
                            ...
                            <div class="col-lg-4">
                              ...
                              <div class="form-group input_new_textbox mb-4">
                                <input type="text" @input="html_tag_notaccept('order_other_reference', $event)" :disabled="isTransfer == '1'" v-model="order_other_reference"
                                  placeholder="Other Reference(s)" class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off" />
                              </div>
                            </div>
                            <div class="col-lg-4">
                              <label class="form_input_lbl">Address</label>
                            ...
                            <div class="col-lg-4">
                              ...
                              <div class="form-group input_new_textbox mb-4">
                                <input type="text" @input="html_tag_notaccept('eway_bill_number', $event)" :disabled="isTransfer == '1'" v-model="eway_bill_number"
                                  placeholder="e-Way Bill Number" class="form-control only-input-disable-all-hide-overlay" value="" autocomplete="off" />
                              </div>
                            </div>
                          </div>
                        </div>
                ...
                <div class="col-12">
                      ...
                      <table class="new_voucher_table">
                        <tr>
                          ...
                          <th style="text-align: right">
                            <div class="new-plus-part">
                              <button v-on:click="insert_ledger()">
                                <i class="fa fa-plus"></i>
                              </button>
                            </div>
                          </th>
                        ...
                        <tr v-for="(ledger, index) in ledger_array" :key="ledger.id">
                          <td>
                            ...
                            <form autocomplete="off" @submit.prevent>
                              <div v-if="ledger.ledger_name !== undefined" class="input-disable-overlay-hide-all-div">
                              ...
                              <div v-else class="input-disable-overlay-hide-all-div">
                                <Dropdown :options="allledgerselection" v-on:selected="addmoreLedger_change($event, index)" v-on:filter="partyfilter" 
                                v-model="ledger.ledger_name" :disabled="isTransfer == '1'" 
                                :maxItem="10" placeholder="Select Ledger Item">
                                </Dropdown>
                              </div>
                          </form>
                          </td>
                          <td>
                            <select name="" id="" :disabled="isTransfer == '1'" class="form-select item_select"
                              v-model="ledger.type" @change="tax_calculation()">
                          ...
                          <td>
                            <input type="number" @keydown="handleKeyDown" :disabled="isTransfer == '1'"
                              ...
                              v-model="ledger.amount" autocomplete="off" v-on:keydown="disableArrowKeys" v-on:wheel="disableMouseWheelScroll" />
                          </td>
                          <td style="text-align: right">
                            <button class="delete_icon_td" :disabled="isTransfer !== '0'"
                  ...
                  <div class="col-lg-6 col-12">
                    <div class="form-group mb-4">
                      <label class="form_input_lbl">Reason For Return</label>

                      <select name="ledger" ref="my_input" id="under" class="form-select input_light_border_dropdown"
                        :disabled="isTransfer == '1'" v-model="reason_type">
                        <option value="">Select Reason For Return</option>
                        <option v-for="(reason, index) in reasonData" :value="reason.reason" :key="index">
                          {{ reason.reason }}
                        </option>
                      </select>
                      <span class="error-validation" v-if="error.reason_error">{{ error.reason_error }}</span>
                    </div>
                  </div>
                  ...
                  <div class="col-12">
                    <div>
                      ...
                      <div class="sales-voucher-textarea">
                        <textarea @input="html_tag_notaccept('narration', $event)" id="narration" :disabled="isTransfer == '1'" v-model="narration" rows="4"
                          placeholder="Narration" cols="50" class="textarea-disable-not-allowed">{{ this.narration }}</textarea>
                      </div>
                    </div>
                  </div>
                </div>
...
export default {
  ...
  if(this.isValidGSTIN == false || this.isValidConsigneeGSTIN == false ){
      ...
      if (voucher_id !== "update") {
        if (this.$route.params.voucher_id === undefined) {
          ...
          if (
            ...
            this.tableRows[index].rate = parseFloat(selection.id).toFixed(2);
          }
        }
        ...
        this.tableRows[index].item_name = selection.stockItemName;
        this.tableRows[index].item_guid = selection.guid;
        this.tableRows[index].baseUnits = selection.baseUnits;
        this.tableRows[index].parent = selection.parent;
        this.tableRows[index].parentGuid = selection.parentGuid;
    ...
    this.party_name = selection.name;
      ...
      if (companyGST == "") {
        ...
        if (partyGST.substring(0, 2) == companyGST.substring(0, 2)) {
    ...
    tax_value_select(event, taxindex = 0, ledgerarray = {}) {
      ...
      if (Object.keys(ledgerarray).length !== 0) {
        // Update case
        console.log(ledgerarray, "ledgerarray ");
        setTimeout(() => {
          ...
          this.taxarray[taxindex].tax_select = ledgerarray.name;
    ...
    add_sales() {
      ...
      if (!this.reason_type) {
        this.error["reason_error"] = "Please select reason for return";
      }
      if (Object.keys(this.error).length === 0) {
        ...
        const data = {
          ...
          gstNatureOfReturn: this.reason_type,
          transferComment: "",
          ...
          eWayBillDetails: [
            {
              ...
              transpoertDetails: [],
              consignorGstIn: "",
              billDate: this.eway_date != "" && this.eway_date != null ? moment(String(this.eway_date)).format("YYYY-MM-DD") : "",
              subType: "",
              consigneeGstIn: "",
              billNumber: this.eway_bill_number,
              consigneeName: "",
              billStatus: "",
              consigneeStateName: "",
              consigneePincode: "",
      ...
      if (this.showledgers == true) {
        this.ledger_array.forEach((number1) => {
          ...
          this.add_more_ledger.push({
            ledgerName: number1.ledger_name,
            descriptions: "",
            ledgerGuid: this.ledger_ledgerGuid,
           transactionType: number1.type,

            isMoreLedgerField: "1",
            ledgerGroup: this.ledger_ledgerGroup,
            ledgerGroupParent: this.ledger_ledgerGroupParent,
            amount: amount,
            billAllocationdetails: [],
            bankAllocationDetails: [],
          });
    ...
    get_sale_data() {
      ...
      Api.getVoucherFullDetailsFromSyncGoo(data, headers)
        .then((response) => {
          ...
          this.ledgerId = response.data.data.partyLedgerGuid;
            ...
            Api.getItemsByCompanyIdGo(data, headers).then((response) => {
              if (response.data.status != "0") {
                for (let [key2, value2] of Object.entries(
                  ...
                  this.itemselected(value2, key, "update",value);
                  break;
                }
              }
            });

            for (let [key1, value1] of Object.entries(value.taxes)) {
              this.taxarray = [];
              this.total_tax += parseFloat(value1.amount);
             
              this.tax_value_select("", key1, value1);
          ...
          for (let [key, value] of Object.entries(
            ...
            if (value.isMoreLedgerField == "1"  && (value.ledgerGroupParent).toLowerCase() != "duties & taxes") {
              ...
              if (value.amount >= 0) {
                // + value
                this.ledger_amount += parseFloat(value.amount);
                value.type = "Credit";
              } else {
                // - value
                this.ledger_amount -= parseFloat(value.amount);
          ...
          this.buyer_regisration_type =
            response.data.data.gstRegistrationType;
          this.buyer_pincode =
            response.data.data.basicBuyerDetails.billToPincode;
          this.buyer_place_of_supply =
            response.data.data.basicBuyerDetails.placeOfSupply;
          this.buyer_gstin = response.data.data.basicBuyerDetails.billToGSTIN;
          this.buyer_address =
            response.data.data.basicSenderAddress;

          this.consignee_name =
            response.data.data.basicConsigneeDetails.shipTo;
          this.consignee_country =
            response.data.data.basicConsigneeDetails.shipToCountry;
          this.consignee_state =
            response.data.data.basicConsigneeDetails.shipToState;
          this.consignee_pincode =
            response.data.data.basicConsigneeDetails.basicConsigneePincode;
          this.consignee_gstin =
            response.data.data.conSigneegstin;
          this.consignee_address =
            response.data.data.basicConsigneeDetails.shipToAddress;

            this.reason_type =
            response.data.data.gstNatureOfReturn;
          this.transfer_return_number =
            response.data.data.vatPartyTransferReturnNumber;
          this.transfer_date =
            response.data.data.vatPartyTransferReturnDate;
          this.narration = response.data.data.narration;

          this.dispatch_delivery_note =
            response.data.data.basicShipDeliveryNote;
          this.dispatch_shipping_date =
            response.data.data.basicShippingDate;
          this.dispatch_doc_no =
            response.data.data.basicShipDocumentNo;
          this.dispatch_despatch_through =
            response.data.data.basicShipedBy;
          this.dispatch_destination =
            response.data.data.basicFinalDestination;
          this.dispatch_bill_no =
            response.data.data.billOfLadingNo;
          this.dispatch_motor_vehicle_no =
            response.data.data.basicShipVesselNo;

          this.order_date =
            response.data.data.basicOrderDate;
          this.order_no =
            response.data.data.basicPurchaseOrderNo;
          this.order_invoice_date =
            response.data.data.basicDateTimeOfInvoice;
          this.order_terms_of_payment =
            response.data.data.basicDueDateOfPymt;
          this.order_other_reference =
            response.data.data.basicOrderRef;
          this.order_details_address =
            response.data.data.basicOrderTerms;
          ...
          this.eway_date =
            response.data.data.eWayBillDetails[0].billDate;
          this.eway_bill_number =
    ...
    update_sales() {
      ...
      if (Object.keys(this.error).length === 0) {
        ...
        if (this.showledgers == true) {
          ...
          this.final_ledgerEntries.forEach((item, index) => {
            if (item.ledgerGroupParent !== "Duties & Taxes" && item.isMoreLedgerField == "1") {
          ...
          eWayBillDetails: [
            {
              ...
              transpoertDetails: [],
              consignorGstIn: "",
              billDate: moment(String(this.eway_date)).format("YYYY-MM-DD"),
              subType: "",
              consigneeGstIn: "",
              billNumber: this.eway_bill_number,
              consigneeName: "",
      ...
      Api.getTransactionTypeGo(data, headers)
        .then((response) => {
          if (response.data.status != "0") {
            ...
            this.reasonData = response.data.data.value;
          }
        })
        .catch((err) => {
          Common.getStatusForActionFailed(err.response.status);
    ...
    InvoiceConfigurations() {
      ...
      Api.getInvoiceConfigurationsGo(data, headers)
        .then((response) => {
          if (response.data.status != "0") {
            this.invoice_type = response.data.data.vouchers.voucherNumberConfiguration.debitnote.isSetDefaultNumber;
          }
        })
        .catch((err) => {
          Common.getStatusForActionFailed(err.response.status);
  ...
  mounted() {
    GoogleAnalytics.debit_note_listing();
