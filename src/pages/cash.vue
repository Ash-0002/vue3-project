                    ...
                    <template v-slot:item.ledgerName="{ item }">
                      <router-link
                        ...
                        :to="{
                          ...
                          params: { v_type: item.masterReservedName },
                        }"
                      >
...
export default {
  ...
  watch: {
    page: function () {
      ...
      this.ledger_report();
      GoogleAnalytics.cash_screen_pagination(this.options.page);
    },

    "options.itemsPerPage"() {
      this.page=1
      this.options.page = 1; 
      this.ledger_report(); // console.log(`hehehe`);
      if (this.call_function == "1") {
        this.call_function = "1";
        GoogleAnalytics.cash_screen_Rows_Per_Page(this.options.itemsPerPage);
      } else {
        this.call_function = "1";
      }
    },
  ...
  methods: {
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.cash_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.cash_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.cash_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.cash_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.cash_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.cash_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.cash_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    update() {
      ...
      GoogleAnalytics.cash_screen_date_filter("Custom");
      // alert(DateRangePicker.startDate);
      this.ledger_report();
      this.date_setting();
    },
    date_setting() {
    ...
    ledger_report(type) {
      ...
      Api.getVouchersSummaryByCashOrBankGo(data, headers)
        .then((response) => {
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.cash_screen_search(this.search);
      this.ledger_report(type, this.groupSelectValue, this.switch_type);
    },
    updateSort(byDesc, event) {
      ...
      this.ledger_report();
      GoogleAnalytics.cash_screen_sorting(this.sortBy, this.orderBy);
