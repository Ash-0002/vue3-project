                          ...
                          />
                                    ...
                                    <ul class="dashboard_sales_dropdown new_search_export_button parties_addbutton_top">
                                      <div v-if="this.source !== 'sf'">
                                        ...
                                        <div>
                                          <li v-if="this.isWrite_isStockItem == true" class="">
                                            <router-link :to="{ name: 'add-items' }" class="new_addnew_button_table" @click="paiduseraddnew()">
                                          ...
                                          <li v-else>
                                            <router-link :to="''" class="new_addnew_button_table" >
                                              <img src="/assets/images/1.0/png/white-add.png" alt="img" class="table-icon-add" />
                            ...
                            <v-data-table :page="page" :pageCount="numberOfPages" :headers="selectedHeaders" 
                                    ...
                                    <template v-slot:item.applicableFrom="{ item }">
                                        <td><p>{{format_date(item.createdAt)}}</p></td>
                                    </template>
                                    ...
                                    <template v-slot:item.action="{ item }">
                                        <td align="center">
                                            ...
                                            <a v-on:click="deleteLedger(item._id,item)" class="delete_icon_reports">
                                              <i v-if=" item.isTransfer == '0' " class="fa fa-trash deleteIcon text-danger"></i>
                                            </a>
                                        </td>
                                    </template>
                                </v-data-table>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            headers: [
                ...
                { text: 'Date', value: 'applicableFrom' },
    ...
    watch: {
        ...
        page: function () {
          ...
          GoogleAnalytics.my_stock_items_reports_screen_pagination(this.options.page);
        },
      ...
      this.options.page = 1; 
        ...
        GoogleAnalytics.my_stock_items_reports_screen_Rows_Per_Page(this.options.itemsPerPage);
      }else{
        this.call_function = "1";
      }
        },
        ...
        options: {
          handler() {
            ...
            GoogleAnalytics.my_stock_items_reports_screen_listing();
    ...
    created() {
        ...
        this.isWrite_isStockItem = JSON.parse(localStorage.getItem('isWrite_isStockItem'));
    ...
    methods: {
      ...
      if (el === "0") {
    GoogleAnalytics.my_stock_items_reports_screen_filter("Pending");
    } else if (el === "1") {
    GoogleAnalytics.my_stock_items_reports_screen_filter("Completed");
    }else if (el === "-1") {
    GoogleAnalytics.my_stock_items_reports_screen_filter("Rejected");
    }else if (el === "3") {
    GoogleAnalytics.my_stock_items_reports_screen_filter("Cancelled");
    }else {
    GoogleAnalytics.my_stock_items_reports_screen_filter("All");
    }
    },
    show() {
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.my_stock_items_reports_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.my_stock_items_reports_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.my_stock_items_reports_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.my_stock_items_reports_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.my_stock_items_reports_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
        ...
        // 2 Year
        GoogleAnalytics.my_stock_items_reports_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        this.$refs.picker.end = new_date_end_date;
          ...
          // max date
          GoogleAnalytics.my_stock_items_reports_screen_date_filter("Max");
          var new_date_start_date = moment(
      ...
      if (days_mounted == "730") {
       ...
       // myLedger(type) {
        readDataFromAPI(type = "", sortBy = "", orderBy = "") {
            ...
            this.myledgers = [];
            Api.getMyStockItems(data, headers)
                .then((response) => {
                if (response.data.message == "AUTHENTICATION_FAILED") {
                    ...
                    for (let [key, value] of Object.entries(response.data.data.list)) {
                        ...
                        response.data.data.list[key].url = "/add-items/"+ value._id;
        ...
        handleBlur() {
            const type = "search";
            GoogleAnalytics.my_stock_items_reports_screen_search(this.search);
        ...
        updateSort(byDesc, event) {
            ...
            this.readDataFromAPI();
            GoogleAnalytics.my_stock_items_reports_screen_sorting(this.sortBy,this.orderBy);
        ...
        deleteLedger(_id,item)
    {
            ...
            // }
            // Api.updateSyncDetails(data, headers)
            // .then((response) => {
        ...
        update() {
            ...
            GoogleAnalytics.my_stock_items_reports_screen_date_filter("Custom");
            this.readDataFromAPI();
            this.date_setting();
