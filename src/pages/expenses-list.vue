      ...
      <div class="row align-items-center">
        <div class="col-xl-3 col-lg-12 col-12">
          <div class="company_name sales_c_name mobile_padding_left_0">
            <h3 id="" class="partyName">{{ this.$route.params.group_name }}</h3>
          </div>
          </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      headers: [
        ...
        { text: "Amount", value: "closingBalance", align: "right" },
      ],
      page: 1,
      totaltopreports: 0,
      numberOfPages: 0,
  ...
  watch: {
    page: function () {
      ...
      this.expenses_list();
      GoogleAnalytics.expenses_list_reports_screen_pagination(
        this.options.page
      );
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.expenses_list_reports_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        GoogleAnalytics.expenses_list_reports_listing();
      },
    },
    deep: true,
  },
  ...
  methods: {
    activate: function (el) {
      if (
        el === "Items Sold By Value" ||
        el === "Items Purchased By Value" ||
        el === "Items Sold By Quantity" ||
        el === "Items Purchased By Quantity"
      ) {
        this.dotshow = 1;
      } else {
        this.dotshow = 0;
      ...
      this.expenses_list();
      GoogleAnalytics.expenses_list_reports_screen_filter(el);
    },
    show() {
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.expenses_list_reports_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.expenses_list_reports_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.expenses_list_reports_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.expenses_list_reports_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.expenses_list_reports_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.expenses_list_reports_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    freeusergeneratePdf() {
      GoogleAnalytics.expenses_list_reports_screen_viewpdf("viewpdf");
    },
    generatePdf() {
      GoogleAnalytics.expenses_list_reports_screen_viewpdf("viewpdf");
      const data = {
        requestFrom: "WEB",
        mainMasterReservedName: this.$route.params.group_name,
      ...
      Api.getPDFForExpensesReportsGo(data, headers)
        .then((response) => {
    ...
    update() {
      ...
      GoogleAnalytics.expenses_list_reports_screen_date_filter("Custom");
      this.expenses_list();
      this.date_setting();
    },
    left_arrow() {
    ...
    expenses_list(type) {
     ...
     const { page, itemsPerPage } = this.options;
      ...
      Api.getExpensesLedgersGo(data, headers)
        .then((response) => {
          if (response.data.message == "AUTHENTICATION_FAILED") {
            localStorage.clear();
            window.location.href = "/";
            ...
            if (response.data.status != "0") {
              for (let [key, value] of Object.entries(
                ...
                response.data.data.list[key].url =
                  "/parties/" + response.data.data.list[key].ledgerGuid;
              }
              ...
              this.totaltopreports = response.data.data.totalCount;
              this.numberOfPages = totalPages;
              this.totalAmountsPer = response.data.data.totalAmountsPer;
              this.totalCountPer = response.data.data.totalCountPer;
            } else {
              this.loading = false;
              this.new_loading = false;
              this.topreports = [];
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.expenses_list_reports_screen_search(this.search);
      this.expenses_list(type);
    },

    updateSort(byDesc, event) {
      this.options.page = this.page;
      // Mapping table headers to API field names
      const sortFieldMap = {
        ledgerName: 'name',
        closingBalance: 'amount'
      };
      if (byDesc == "by") {
        this.sortBy = sortFieldMap[event] || event;
        this.orderBy = "-1";
      } else if (byDesc == "desc") {
        if (event == true) {
          this.orderBy = "1";
      ...
      this.expenses_list();
      GoogleAnalytics.expenses_list_reports_screen_sorting(
        this.sortBy,
        this.orderBy
      );
