            ...
            <div class="company_balance">
              <p>
                ...
                <strong><span id="closing_balance">
                    ...
                    {{ Math.abs(closingBalance) | formatNumberParties }}</span></strong>
              </p>
...
export default {
  ...
  methods: {
    ...
    generatePdf() {
      ...
      if (this.data_filters === "") {
        ...
        if (this.$route.params.v_type == "Sales") {
          this.filters = ["Sales", "Credit Note"];
          this.data_filters = this.$route.params.v_type;
        }
        else if (this.$route.params.v_type == "Sales1") {
          this.filters = ["Sales"];
          this.data_filters = "Sales";
        }
        else if (this.$route.params.v_type == "Credit Note1") {
          this.filters = ["Credit Note"];
          this.data_filters = "Credit Note";
        }
        else if (this.$route.params.v_type == "Purchase") {
          this.filters = ["Purchase", "Debit Note"];
          this.data_filters = this.$route.params.v_type;
        }
        else if (this.$route.params.v_type == "Purchase1") {
          this.filters = ["Purchase"];
          this.data_filters = "Purchase";
        }
        else if (this.$route.params.v_type == "Debit Note1") {
          this.filters = ["Debit Note"];
          this.data_filters = "Debit Note";
        }
        else if (this.$route.params.v_type == "Receipt") {
          this.filters = ["Receipt"];
          this.data_filters = this.$route.params.v_type;
          ...
          this.$route.params.v_type === undefined ||
          this.this.$route.params.v_type == ""
        ) {
          this.filters = [];
        } else {
          this.filters = [this.$route.params.v_type];
        }
      } else {
        if (this.data_filters === "All Voucher") {
          this.filters = [];
        } else {
          this.filters = [this.data_filters];
        }
      }
      ...
      if (
        this.getLedgerData.parent.toLowerCase() ==
        "Bank Accounts".toLowerCase() ||
        this.getLedgerData.parent.toLowerCase() ==
        "Bank OD A/c".toLowerCase() ||
        this.getLedgerData.parent.toLowerCase() ==
        "Bank OCC A/c".toLowerCase() ||
        this.getLedgerData.parent.toLowerCase() ==
        "Cash-in-Hand".toLowerCase() ||
        this.getLedgerData.parent.toLowerCase() == "Cash-in-hand".toLowerCase()
      ) {
        // alert("inside");
        if (
          this.getLedgerData.parent.toLowerCase() ==
          ...
          this.pdfVoucherType = "Contra";
        } else {
          this.pdfVoucherType = "Payment";
        }
        let apiForPdf = ''
        const data = {
          company_id: localStorage.getItem("companyId"),
    ...
    activate: function (el) {
      ...
      //   el = "All Voucher";
      // }
      if (el == 'Sales Order' || el == 'Purchase Order') {
        this.$router.push({
        ...
        .then((response) => {
          ...
          if (responseData == "200") {
            ...
            if (response.data.data.ledgerMobile == "") {
              response.data.data.w_show = 0;
              response.data.data.w_url = "";
            } else {
              response.data.data.w_show = 1;
              response.data.data.w_url =
                "https://api.whatsapp.com/send?phone=" +
                response.data.data.ledgerMobile.replace("+91 ", "");
            }
    ...
    readDataFromAPI(guid, filters = "") {
      ...
      if (this.data_filters === "") {
        if (this.$route.params.v_type !== undefined && this.$route.params.v_type !== null && this.$route.params.v_type !== "") {
          if (this.$route.params.v_type == "Debit Note1") {
            filters = ["Debit Note"];
          }
          else {
            filters = [this.$route.params.v_type];
          }
      ...
      if (
        ...
        this.active_el = this.$route.params.v_type;
        this.check_count = "1";
      }
      ...
      Api.getLedgerSummary(data, headers)
        ...
        .then((response) => {
          ...
          if (responseData == "200") {
            ...
            this.closingBalance = Math.abs(response.data.data.closingBalance);
