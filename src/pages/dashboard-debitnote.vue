                  ...
                  <v-data-table :page="page" :pageCount="numberOfPages" :headers="selectedHeaders" :items="salecustomer"
                    ...
                    <template v-slot:item.name="{ item }">
                      <td>
                        ...
                        <div>
                          <router-link :to="`${item.url}`" class="onclick_details">{{ item.ledgerName }}</router-link>
                        </div>
                      </td>
                    </template>
                ...
                <div v-if="bill_view" class="show_entries_for_table new_show_entries_for_table">
                  <v-data-table :page="page" :pageCount="numberOfPages" :headers="selected_bill_headers"
                    ...
                    <template v-slot:item.date="{ item }">
                      <td>
                        <router-link :to="`${item.url}`" class="onclick_details">{{ item.date }}</router-link>
                      </td>
                    </template>
                    ...
                    <template v-slot:item.voucherNumber="{ item }">
                      <td>
                        <p>
                          <router-link :to="`${item.url}`" class="onclick_details">{{ item.voucherNumber
                          }}</router-link>
                        </p>
                      </td>
                    </template>
...
export default {
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.purchase_screen_pagination(this.options.page);
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.purchase_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.purchase_listing();
      },
    },
    deep: true,
  },
  ...
  methods: {
    freeusergeneratePdf() {
      GoogleAnalytics.purchase_screen_viewpdf("viewpdf");
    },
    generatePdf() {
      GoogleAnalytics.purchase_screen_viewpdf("viewpdf");
      const data = {
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.purchase_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.purchase_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.purchase_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.purchase_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.purchase_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.purchase_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.purchase_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    sale_data(type, group_name = "Ledger", switch_type = "Gross", v_type = "") {
      ...
      if (this.groupSelectValue === "Ledger") {
        ...
        if (v_type != "") {
          ...
          this.use_v_type = ["Purchase", "Debit Note"];
        }
        const data = {
          _userId: localStorage.getItem("customerId"),
          company_id: localStorage.getItem("companyId"),
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.purchase_screen_search(this.search);
    ...
    updateSort(byDesc, event) {
      ...
      GoogleAnalytics.purchase_screen_sorting(this.sortBy, this.orderBy);
    ...
    update() {
      ...
      GoogleAnalytics.purchase_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
    ...
    vouchertypeClick(v_type) {
      console.log(v_type, 'voucher type clicked')
