                    ...
                    <template v-slot:item.totalAmount="{ item }">
                      <td align="right">
                        <p class="amount_closing">
                          <span class="font_sans">{{ symbol }}</span>
                          {{
                            parseFloat(Math.abs(item.amount))
                              | formatNumberSales
                          }}
                        </p>
                      </td>
...
export default {
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.receipt_month_screen_pagination(this.options.page);
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.receipt_month_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.receipt_month_listing();
      },
    },
    deep: true,
  },
  ...
  methods: {
    ...
    freeusergeneratePdf() {
      GoogleAnalytics.receipt_month_screen_viewpdf("viewpdf");
    },
    VoucherPdf(v_id){
      const data = {
        requestFrom: "WEB",
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.receipt_month_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.receipt_month_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.receipt_month_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.receipt_month_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.receipt_month_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.receipt_month_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.receipt_month_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.receipt_month_screen_search(this.search);
    ...
    updateSort(byDesc, event) {
      ...
      GoogleAnalytics.receipt_month_screen_sorting(this.sortBy, this.orderBy);
    ...
    update() {
      ...
      GoogleAnalytics.receipt_month_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
