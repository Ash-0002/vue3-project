                  ...
                  >
                    ...
                    <template v-slot:item.debit="{ item }">
                      <td>
                        ...
                        {{ item.debit | formatNumberParties }}
                      </td>
                    </template>
                    <template v-slot:item.credit="{ item }">
                      <td>
                        <span class="font_sans">{{ symbol }}</span>
                        {{ item.credit | formatNumberParties }}
                      </td>
                    </template>
                  </v-data-table>
                </div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      headers: [
        ...
        { text: "Debit", value: "debit", align: "left", sortable: false },
        { text: "Credit", value: "credit", align: "left", sortable: false },
      ],
      page: 1,
      totalclosing: 0,
      numberOfPages: 0,
  ...
  watch: {
    page: function () {
      ...
      this.reportDetails();
      GoogleAnalytics.reportdetails_reports_screen_pagination(
        this.options.page
      );
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.reportdetails_reports_screen_Rows_Per_Page(
 ...
 '$route'() {
    ...
    options: {
      handler() {
        GoogleAnalytics.reportdetails_reports_screen_listing();
      },
    },
    deep: true,
  },
  ...
  methods: {
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.reportdetails_reports_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.reportdetails_reports_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.reportdetails_reports_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.reportdetails_reports_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.reportdetails_reports_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.reportdetails_reports_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.reportdetails_reports_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    update() {
      ...
      GoogleAnalytics.reportdetails_reports_screen_date_filter("Custom");
      this.reportDetails();
      this.date_setting();
    },
    left_arrow() {
    ...
    reportDetails(type) {
      ...
      const data = {
        ...
        groupName: this.$route.params.name === "Primary" ? "#4; Primary" : this.$route.params.name,
      };
          ...
          if (response.data.message == "AUTHENTICATION_FAILED") {
            ...
            if (response.data.status != "0") {
              for (let [key, value] of Object.entries(
                ...
                response.data.data.list[key].credit = Math.abs(
                  response.data.data.list[key].totalCreditAmount
                );
                response.data.data.list[key].debit = Math.abs(
                  response.data.data.list[key].totalDebitAmount
                );
                if (value.hasChild === 1) {
                  // Group URL
                  value.parent = value.parent.replace("#4; ", "");
                  response.data.data.list[key].url =
                    "/reportDetails/" + encodeURIComponent(value.parent.replace("#4;", ""));
                } else {
                  // Direct Redirect to item page
                  response.data.data.list[key].url = "/parties/" + value.ledgerGuid;
                }
              }
  ...
  mounted() {
    this.symbol = localStorage.getItem("symbol");
    this.$nextTick(this.reportDetails);
    if (
