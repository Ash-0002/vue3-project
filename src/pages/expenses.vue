              ...
              <v-app id="inspire">
                <div class="row">
                  ...
                  <div class="col-lg-9 col-12">
                    <ul class="parties_ul_with_filter new_search_export_button">
                     <li>
                        <v-select
                          v-model="value"
                          :items="headers"
                  ...
                  >
                    ...
                    <template v-slot:item.ledgerName="{ item }">
                      <div v-if="isPaid == '' || isPaid == null">
                        <b-button class="vch_num_all_btn">{{
                          item.ledgerName
                        }}</b-button>
                      </div>
                      <div v-else>
                        <router-link
                          ...
                          :to="{
                            name: 'Expenses List',
                            params: { group_name: item.masterReservedName },
                          }"
                          >{{ item.ledgerName }}
                        </router-link>
                      </div>
...
export default {
  ...
  watch: {
    ...
    page: function () {
      ...
      this.expenses_report();
      GoogleAnalytics.expenses_reports_screen_pagination(this.options.page);
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.expenses_reports_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        GoogleAnalytics.expenses_reports_listing();
      },
    },
    deep: true,
  },
  ...
  methods: {
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.expenses_reports_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.expenses_reports_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.expenses_reports_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.expenses_reports_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.expenses_reports_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.expenses_reports_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
    ...
    update() {
      ...
      GoogleAnalytics.expenses_reports_screen_date_filter("Custom");
      this.expenses_report();
      this.date_setting();
    },
    left_arrow() {
    ...
    expenses_report(type) {
      ...
      Api.getLedgerGroupForExpenses(data, headers)
        .then((response) => {
          const responseStatus = Common.getStatusForActionFailed(
            ...
            if(responseStatus == '200') {
              ...
              this.expenses = response.data.data.list;
              this.totalexpenses = response.data.data.totalCount;
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.expenses_reports_screen_search(this.search);
      this.expenses_report(type);
    },
    ...
    updateSort(byDesc, event) {
      ...
      this.expenses_report();
      GoogleAnalytics.expenses_reports_screen_sorting(
        this.sortBy,
        this.orderBy
      );
