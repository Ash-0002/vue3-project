...
<router-link :to="{name: 'items-listing'}"> <i class="fa fa-chevron-left"></i></router-link>
</div>
...
<div class="company_name new_title_company_name">
                ...
                <div class="item_closing_stock_left">
                  <p>Avg Pur Rate: <strong><span class="font_sans">{{symbol}}</span>{{avgRate |
                ...
                <div class="col-xl-7 col-lg-12 col-12">
                    <ul class="new_set_dropdown_with_gross">
                          ...
                          <li><router-link class="headers" :to="{name: 'items',params: { id: this.$route.params.id,guid : this.$route.params.guid} }"> Summary </router-link></li>
                          <li><router-link class="headers" :to="{name: 'items-customers',params: { id: this.$route.params.id,guid : this.$route.params.guid} }"> Customers </router-link></li>
                          <li><router-link class="headers" :to="{name: 'items-supplers',params: { id: this.$route.params.id,guid : this.$route.params.guid} }"> Suppliers </router-link></li>
                         </ul>
                            </li>
                           <li>
          ...
          <div class="col-12">
                  ...
                  <div class="show_entries_for_table new_show_entries_for_table">  
                      <v-data-table :page="page" :pageCount="numberOfPages"
                          ...
                          :sort-by="null"
                          :sort-desc="false"
                            @update:sort-by="updateSortCustomers('by', $event)"
                            @update:sort-desc="updateSortCustomers('desc', $event)" disable-pagination item-key="table_header_index"
                          ...
                          <template v-slot:item.itemName="{ item }">
                            ...
                            <div v-else>
                              <router-link class="onclick_details parties_inner_a"
                                  :to="{name: 'vouchers-listing', params: {ledger_name: item.ledgerName,ledgerGuid : item.ledgerGuid,stockItemGuid : stockItemGuid,isSales : true},query: { type: 'Sales' }}">{{item.ledgerName}}
                              </router-link>
                            </div>
...
export default {
  ...
  watch: {
    page: function () {
       ...
       GoogleAnalytics.items_customers_screen_pagination(this.options.page);
      ...
      this.options.page = 1; 
       this.get_Item_details();
        ...
        GoogleAnalytics.items_customers_screen_Rows_Per_Page(this.options.itemsPerPage);
      }else{
        this.call_function = "1";
      }
      },
    ...
    options: {
        handler() {
            ...
            GoogleAnalytics.items_customers_listing();
        },
    },
    deep: true,
  },
  methods: {
    ...
    activatedate:function(date){
        if(date === "oned"){
            // one day
            GoogleAnalytics.items_customers_screen_date_filter("1D");
            ...
            // one week
            GoogleAnalytics.items_customers_screen_date_filter("1W");
            ...
            // one month
            GoogleAnalytics.items_customers_screen_date_filter("1M");
            ...
            // 3 month 
            GoogleAnalytics.items_customers_screen_date_filter("3M");
            ...
            // 1 Year
            GoogleAnalytics.items_customers_screen_date_filter("1Y");
            ...
            // 2 Year
            GoogleAnalytics.items_customers_screen_date_filter("2Y");
            ...
            this.$refs.picker.end = new_date_end_date;
                ...
                // max date
                GoogleAnalytics.items_customers_screen_date_filter("Max");
    ...
    update() {
          ...
          GoogleAnalytics.items_customers_screen_date_filter("Custom");
        this.getItemList();
        this.date_setting();
    },
      ...
      Api.getItemByIdGo(data, headers)
        .then(response => {
          ...
          if(responseData == '200'){
            ...
            this.stockItemGuid = response.data.data.guid;
          }
        })
        .catch((err) => {
            Common.getStatusForActionFailed(err.response.status)
    ...
    handleBlur() {
        const type = "search";
        GoogleAnalytics.items_customers_screen_search(this.search);
        this.get_Item_details(type);
    },
    paginationClicked() {
    },
    updateSortCustomers(byDesc, event) {
      this.options.page = this.page;
      ...
      if (byDesc == "by") {
        ...
        GoogleAnalytics.items_customers_screen_sorting(this.sortBy,this.orderBy);
    }
  },
