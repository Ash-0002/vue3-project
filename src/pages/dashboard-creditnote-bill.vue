              ...
              <v-app id="inspire">
                ...
                <div v-if="bill_view" class="show_entries_for_table new_show_entries_for_table">
                  <v-data-table :page="currentPage" :pageCount="numberOfPages" :headers="selected_bill_headers"
                    ...
                    :items-per-page="this.$store.state.Web1_Store.CreditNote_Bill_itemsPerPage"
                    :server-items-length="totalsalecustomer" :loading="loading" class="elevation-1" must-sort
                    :sort-by="null" :sort-desc="false" @update:sort-by="updateSort('by', $event)"
                    @update:sort-desc="updateSort('desc', $event)" ref="table" :hide-default-footer="true"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      page: this.$store.state.Web1_Store.CreditNote_Bill_page,
      totalsalecustomer: 0,
      numberOfPages: 0,
      salecustomer: [],
      value: [],
  ...
  watch: {
    ...
    "options.itemsPerPage"() {
      setTimeout(() => {
        const StartIndex = (this.$store.state.Web1_Store.CreditNote_Bill_page - 1) * this.$store.state.Web1_Store.CreditNote_Bill_itemsPerPage
        const endIndex = StartIndex + this.$store.state.Web1_Store.CreditNote_Bill_itemsPerPage
        const Difference = endIndex - this.totalsalecustomer

        if (Difference > this.totalsalecustomer || Difference == this.totalsalecustomer) {
          ...
          this.page = this.$store.state.Web1_Store.CreditNote_Bill_page
          this.options.page = this.page;
  ...
  computed: {
    currentPage() {
      return this.$store.state.Web1_Store.CreditNote_Bill_page;
    },
  ...
  methods: {
    ...
    StoredPage(newValue) {
      this.$store.dispatch('Web1_Store/setCreditNoteBillPage', newValue.page)


    },
    updateItemsPerPage(newValue) {
      this.$store.dispatch('Web1_Store/setCreditNoteBillItemsPerPage', newValue)
    ...
    activate: function (el) {
      ...
      if (el === "Ledger") {
        this.$router.push({ name: "dashboard-creditnote" });
      } else if (el === "Stock Item") {
        this.$router.push({ name: "dashboard-creditnote-stock-item" });
      } else if (el === "Month") {
        this.$router.push({ name: "dashboard-creditnote-month" });
      } else if (el === "Bill") {
        this.$router.push({ name: "dashboard-creditnote-bill" });
      } else if (el === "Ledger Group") {
        this.$router.push({ name: "dashboard-creditnote-ledger-group" });
      } else if (el === "Voucher Type") {
        this.$router.push({ name: "dashboard-creditnote-voucher-type" });
      } else if (el === "Stock Category") {
        this.$router.push({ name: "dashboard-creditnote-stock-category" });
      } else if (el === "Stock Group") {
        this.$router.push({ name: "dashboard-creditnote-stock-group" });
      }
      this.active_el = el;
