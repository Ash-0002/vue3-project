...
export default {
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.purchaseorder_stock_category_screen_pagination(
        this.options.page
      );
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.purchaseorder_stock_category_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.purchaseorder_stock_category_listing();
      },
    },
    deep: true,
  },
  ...
  methods: {
    freeusergeneratePdf() {
      GoogleAnalytics.purchaseorder_stock_category_screen_viewpdf("viewpdf");
    },
    generatePdf() {
      GoogleAnalytics.purchaseorder_stock_category_screen_viewpdf("viewpdf");
      const data = {
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.purchaseorder_stock_category_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.purchaseorder_stock_category_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.purchaseorder_stock_category_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.purchaseorder_stock_category_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.purchaseorder_stock_category_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.purchaseorder_stock_category_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.purchaseorder_stock_category_screen_date_filter("Max");
        var new_date_start_date = moment(
          ...
          .then((response) => {
            ...
            if (responseData == "200") {
              for (let [key, value] of Object.entries(
                ...
                response.data.data.list[key].name = response.data.data.list[
                  key
                ].stockItemName.replace("#4;", "");
                response.data.data.list[key].url =
                  "/stock-category-item/" +
                  response.data.data.list[key].stockCategoryGuid +
                  "/" +
                  btoa(response.data.data.list[key].stockItemName) +
                  "/Purchase Order";
    ...
    updateSort(byDesc, event) {
      ...
      GoogleAnalytics.purchaseorder_stock_category_screen_sorting(
        this.sortBy,
        this.orderBy
      );
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.purchaseorder_stock_category_screen_search(this.search);
    ...
    update() {
      ...
      GoogleAnalytics.purchaseorder_stock_category_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
