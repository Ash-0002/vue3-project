    ...
    <div class="TopNewBackEntrySectionRechanges mt-3 ps-1 pe-1">
      <div class="TopInnerBackEntrySectionRechanges">
        <div class="qrl_error_box p-0 AddPaddingOnlyError" v-if="stockItemsError">
          ...
          <!-- <p v-if="otherreason_error.pricelevelSearchQuery">{{otherreason_error.pricelevelSearchQuery}}</p> -->
        </div>
        <div class="row">
                ...
                <div class="mt-3 border-top pt-3 pb-3">
                  ...
                  <!-- <div class="ReferencetypesearchRechanges">
                    ...
                    <input type="text" v-model="referenceSearchQuery" @focus="ReferencetoggleDropdown(true)"
                      ...
                      :disabled="isDisable" />
                    <button  :disabled="isDisable" v-if="referenceSearchQuery" @click="ReferenceclearSearch" class="inputcrossiconRechanges">
                      <i class="fa fa-times"></i>
                    </button>
...
:disabled="this.isVoucherEditable || (this.isTransfer == 1 && this.fromMyVoucher)" :disabled-dates="disabledBookDates"></Datepicker>
          ...
          <div class="col-lg-4 col-12 mb-3">
            <div class="OtherTypeSearchDropdownRechanges">
              <span class="borderlinetextdropdownRechanges backgroundgradientRechanges">Reason For Return</span>
              <input ref="pricelevelInput" type="text" v-model="pricelevelSearchQuery" @click='handleReasonHelperData'
                ...
                @keydown.enter.prevent="selectActiveOption('pricelevel')" placeholder="Select Reason for return"
                 />
              <span class="error-validation" v-if="otherreason_error.pricelevelSearchQuery">{{
                otherreason_error.pricelevelSearchQuery }}</span>
              ...
              <ul v-if="pricelevelDropdownOpen" class="SetScrollbarWidthSize">
                <li v-for="(option, index) in pricelevelFilteredOptions" :key="option.reason" :data-id="option.id"
          ...
          <div class="col-lg-4 col-12 mb-2">
            <!-- Transfer Return Name Input -->
            <div class="OtherTypeSearchDropdownRechanges">
              <span class="borderlinetextdropdownRechanges backgroundgradientRechanges">Transfer Return Number</span>
              <input type="text" v-model="transfer_return_number"  />
            </div>
          </div>
          <div class="col-lg-4 col-12 mb-2">
            <!-- Transfer Return Name Input -->
            <div class="newbackentryinvoicedateRechanges">
              <label class="VoucherDateRechanges">Transfer Date</label>
              <div class="datepicker_image_set">
                <Datepicker v-model="transfer_date" format="dd MMM yyyy" placeholder="Transfer Date"
                  ></Datepicker>
                <span class="sales_date_img_new">
                  <img src="/assets/images/1.0/sales/calendar.svg" class="img-fluid" />
                </span>
          ...
          <tr v-for="(addnewitem, index) in AddNewItemList" :key="addnewitem.id">
            ...
            <td class="text-left">
              ...
              <div class="TableItemsserachableDropdownRechanges">
                <input ref="tableitemsInput" type="text" v-model="AddNewItemList[index].tableitemsSearchQuery"
                  @focus="toggleTableItemsDropdown('Items', index, true)" @blur="toggleTableItemsDropdown(index, false)"
            ...
            <!-- <td class="text-center">
              <div class="TaxInclusivecheckboxRechanges">
                <input  @click="taxInclusive_calculation($event,index)"  type="checkbox" id="customCheckbox" v-model="addnewitem.isChecked" class="custom-checkbox" />
              </div>
            </td> -->
            <td class="text-center">
...
option, index
                    ...
                    <div class="col-lg-4 col-12 mb-3">
                      <div class="AdvancedSettingInputRechanges position-relative">
                        ...
                        <input type="text" placeholder="GSTIN/UIN" @keydown="handleKeyDown"
                          v-on:keydown="disableArrowKeys" v-on:wheel="disableMouseWheelScroll" v-model="BuyerGstin"
...
option, index
                    ...
                    <div class="col-lg-6 col-12 mb-3">
                      <div class="AdvancedSettingInputRechanges position-relative">
                        <span class="AdvancedSettingspanRechanges">GSTIN/UIN</span>
                        <input type="text" placeholder="GSTIN/UIN" @keydown="handleKeyDown"
                          v-on:keydown="disableArrowKeys" v-on:wheel="disableMouseWheelScroll" ref="consigneeGSTInput"
                          v-model="consigneeGstin" @keydown.enter="focusConsigneeCountryInput"
                           @input="validateGSTIN" :disabled="isDisable"/>
                      </div>
                    </div>
                  </div>
...
export default {
  ...
  methods: {
    clearReferenceNumber() {
     ...
     revertBack() {
       if(this.voucherSyncId){
        GoogleAnalytics.DebitNote_Revert_Back_Voucher();
      }
      else{
        GoogleAnalytics.DebitNote_Delete_Voucher();
      }
        const data = {
    ...
    validation() {
      ...
      //   this.button_error["price_level"] = "Reason for return is required";
      // }
      ...
      Api.getInvoiceConfigurationsGo(data, headers)
        .then((response) => {
          ...
          if (responseData == "200") {
            ...
            this.invoice_type =
              CommonResponse.voucherNumberConfiguration.debitnote.isSetDefaultNumber;
            this.isEwayStatus = CommonResponse.isEwayStatus;
            this.customizeVoucherNumber =
              CommonResponse.voucherNumberConfiguration.debitnote.customizeVoucherNumber;
            ...
            if (this.$route.params.voucher_id !== undefined) {
              this.customizeVoucherNumber.isDebitNoteNumber =
                Number(response.data.data.vouchers.voucherNumberConfiguration.debitnote.customizeVoucherNumber.number) - 1;
            }
            else {
              this.customizeVoucherNumber.isDebitNoteNumber =
                response.data.data.vouchers.voucherNumberConfiguration.debitnote.customizeVoucherNumber.number;
    ...
    async reasonlist() {
      ...
      await Api.getTransactionTypeGo(data, headers)
        .then((response) => {
          if (response.data.status != "0") {
            for (let [key, value] of Object.entries(response.data.data.value)) {
              response.data.data.value[key].reason = response.data.data.value[
                key
              ].reason.replace("#4; ", "");
            }
            this.pricelevelOptions = response.data.data.value;
            this.pricelevelFilteredOptions = this.pricelevelOptions;
          }
        })
        .catch((err) => {
    ...
    handleReasonHelperData() { 
      if(this.pricelevelFilteredOptions.length == 0) {
        this.reasonlist();
      }
    },
  ...
  AmountChangeFunction(index){
    ...
    updateAmount(index) {
      this.stockItemsError.itemquantity = "";
    ...
    add_Sales() {
      ...
      if (this.pricelevelSearchQuery == "") {
        this.otherreason_error["pricelevelSearchQuery"] =
          "Please Select Reason For Return";
        this.stockItemsError["pricelevelSearchQuery"] =
          "Please Select Reason For Return";
      }
      ...
      if (Object.keys(this.stockItemsError).length === 0) {
        ...
        this.final_ledgerEntries.forEach((item, index) => {
          if (
            item.ledgerGroupParent !== "Duties & Taxes" &&
        ...
        const data = {
          ...
          vatPartyTransferReturnDate: this.transfer_date,
          vatPartyTransferReturnNumber: this.transfer_return_number,
          gstNatureOfReturn: this.pricelevelSearchQuery,
      ...
      apiCall(data, headers)
        .then((response) => {
         ...
         this.buyerAddress = response.data.data.basicBuyerDetails.billToAddress;
          ...
          this.transfer_date = response.data.data.vatPartyTransferReturnDate;
          this.transfer_return_number =
            response.data.data.vatPartyTransferReturnNumber;
          this.pricelevelSearchQuery = response.data.data.gstNatureOfReturn;
    ...
    async getLedgerParentType(type) {
     ...
     cancelOrDeleteVoucher() {
       GoogleAnalytics.DebitNote_Cancel_Voucher();
  ...
  mounted() {
    ...
    if (this.buyersnameSearchQuery == '') {
      this.buyersnameSearchQuery = this.CompanyNameData
    }
