...
export default {
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.sales_stock_category_screen_pagination(this.options.page);
    },
    ...
    "options.itemsPerPage"() {
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.sales_stock_category_screen_Rows_Per_Page(
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.sales_stock_category_listing();
      },
    },
    deep: true,
  },
  ...
  methods: {
    freeusergeneratePdf() {
      GoogleAnalytics.sales_stock_category_screen_viewpdf("viewpdf");
    },
    generatePdf() {
      GoogleAnalytics.sales_stock_category_screen_viewpdf("viewpdf");
      const data = {
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.sales_stock_category_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.sales_stock_category_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.sales_stock_category_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.sales_stock_category_screen_date_filter("3M");
        var new_date_start_date = moment(
          String(
        ...
        var new_date_end_date = moment(
          String(new Date(new Date().getFullYear(), new Date().getMonth(), 0))
        ).format("yyyy-MM-DD");
        this.dateRange.startDate = new_date_start_date;
        this.dateRange.endDate = new_date_end_date;
        this.$refs.picker.start = new_date_start_date;
        ...
        // 1 Year
        GoogleAnalytics.sales_stock_category_screen_date_filter("1Y");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 2 Year
        GoogleAnalytics.sales_stock_category_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.sales_stock_category_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.sales_stock_category_screen_search(this.search);
    ...
    updateSort(byDesc, event) {
      ...
      this.sale_data("", this.groupSelectValue, this.switch_type);
      GoogleAnalytics.sales_stock_category_screen_sorting(
        this.sortBy,
        this.orderBy
      );
    ...
    update() {
      ...
      GoogleAnalytics.sales_stock_category_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
