              ...
              <v-app id="inspire">
                ...
                >
                  <v-data-table
                    ...
                    dense
                    :items-per-page="this.$store.state.Web1_Store.Sales_Receipt_itemsPerPage"
...
export default {
  ...
  data() {
    ...
    return {
      ...
      page: this.$store.state.Web1_Store.Sales_Receipt_page,
      totalsalecustomer: 0,
      numberOfPages: 0,
      salecustomer: [],
      value: [],
  ...
  watch: {
    page: function () {
      ...
      this.sale_data();
      GoogleAnalytics.receipt_screen_pagination(this.options.page);
    },

    "options.itemsPerPage"() {
      setTimeout(() => {
      const StartIndex = (this.$store.state.Web1_Store.Sales_Receipt_page - 1) * this.$store.state.Web1_Store.Sales_Receipt_itemsPerPage
      const endIndex = StartIndex +  this.$store.state.Web1_Store.Sales_Receipt_itemsPerPage 
     ...
     this.page= this.$store.state.Web1_Store.Sales_Receipt_page
     this.options.page = this.page; 
      ...
      if (this.call_function == "1") {
        ...
        GoogleAnalytics.receipt_screen_Rows_Per_Page(this.options.itemsPerPage);
      } else {
        this.call_function = "1";
      }
    },
    ...
    options: {
      handler() {
        ...
        GoogleAnalytics.receipt_listing();
      },
    },
    deep: true,
  },
  ...
  computed:{
    currentPage() {
      return this.$store.state.Web1_Store.Sales_Receipt_page;
    },

  },
  methods: {
    freeusergeneratePdf() {
      GoogleAnalytics.receipt_screen_viewpdf("viewpdf");
    },
    StoredPage(newValue){
      this.$store.dispatch('Web1_Store/setSalesReceiptPage', newValue.page)
      

    },
    updateItemsPerPage(newValue){
      this.$store.dispatch('Web1_Store/setSalesReceiptItemsPerPage', newValue)
    ...
    activatedate: function (date) {
      if (date === "oned") {
        // one day
        GoogleAnalytics.receipt_screen_date_filter("1D");
        ...
        // one week
        GoogleAnalytics.receipt_screen_date_filter("1W");
        ...
        // one month
        GoogleAnalytics.receipt_screen_date_filter("1M");
        ...
        // 3 month
        GoogleAnalytics.receipt_screen_date_filter("3M");
        var new_date_start_date = moment(String(new Date()))
          .add(-3, "M")
          .format("yyyy-MM-DD");
        var new_date_end_date = moment(String(new Date())).format("yyyy-MM-DD");
        ...
        // 1 Year
        GoogleAnalytics.receipt_screen_date_filter("1Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // 2 Year
        GoogleAnalytics.receipt_screen_date_filter("2Y");
        var new_date_start_date = moment(
          String(
            new Date(
              String(localStorage.getItem("first_date_range_start_date"))
        ...
        // max date
        GoogleAnalytics.receipt_screen_date_filter("Max");
        var new_date_start_date = moment(
    ...
    handleBlur() {
      const type = "search";
      GoogleAnalytics.receipt_screen_search(this.search);
    ...
    updateSort(byDesc, event) {
      ...
      GoogleAnalytics.receipt_screen_sorting(this.sortBy, this.orderBy);
    ...
    update() {
      ...
      GoogleAnalytics.receipt_screen_date_filter("Custom");
      this.sale_data("", this.groupSelectValue, this.switch_type);
      this.date_setting();
    },
    date_setting() {
